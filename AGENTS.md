...
- 同一候选中顺序构建 HttpApi、DataWorker、RagWorker 和 migration 时，每个服务的 `dotnet publish` 必须使用服务私有 `--artifacts-path`，且该根必须位于统一输出目录而不是 macOS 临时 detached source worktree；禁止复用共享 `bin/obj`、混用 `/private/var` 与 `/var` 路径别名或依赖构建顺序，避免一个入口点的依赖图污染另一个入口点。
