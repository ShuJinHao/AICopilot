...
- AICopilot 架构边界由 `AI-ARCH-001`、编译型 Analyzer、AnalyzerTests、ArchitectureTests 和项目图严格阻断；不得弱化或绕过，但也不得把它扩张为对业务测试源码或 case roster 的字节级冻结。
- Unit/Aggregate/Application/Workflow/Contract/Persistence/HttpIntegration/EndToEnd/Deployment/GoldenEval/UI 测试可随生产功能同批新增、修改、迁移和删除。inventory/baseline 只防止静默丢失、漏跑、Skip 和质量回退，不得恢复 Phase 0 冻结、transition、waiver、receipt 或其他测试变更授权机制；退役功能的专属测试必须同批物理删除。
- 验证按“受影响项目/定向测试 → 所属 runner → 候选稳定后一次完整 required”执行；只有 Cloud provider/consumer 契约真实变更时才进入跨仓联合验收。
...
- 旧测试声明迁移账本已经退出活动 CI；历史迁移结果只保留在 Git 历史，不得重新用于绑定当前业务测试方法、限制正常增删改移或充当额外授权入口。
