...
4. 入口通过一次 SSH 发送私有 digest-bound request；migration 请求必须包含五服务完整闭包，稳定 Runner 串行执行全部旧 runtime quiesce、最终备份/checksum、started marker、migration、`--no-deps` rollout、健康检查和 history。migration 只有在 compose 成功、容器真实退出码为 `0`、并出现与本次 invocation 精确绑定的成功标记后才放行 runtime；started 后失败禁止自动恢复旧 runtime并阻断重试。
...
服务器必须预先具备 Harbor pull 凭据；本机构建脚本按需执行 `docker login`，服务器 `deploy-release.sh` 本身只重写所选应用镜像 tag并执行 `docker compose pull`，不伪装成会自动登录 Harbor。全量或按需发布都必须先停稳四个旧 runtime，再生成最终完整 PostgreSQL dump 和 SHA-256；启动 `aicopilot-migration` 前必须把 invocation、候选、plan、备份路径/checksum 和 transaction 目录原子写入 `releases/migration-states/<invocation>.env` 的 `migration-started` 状态，任何遗留 started 状态都阻断重试。脚本在启动 HttpApi/DataWorker/RagWorker/Web 之前同时确认 compose 成功、migration 容器真实退出码为 `0`、与本次 invocation 绑定的成功标记存在，再检查模型和 Embedding API key 已全部迁移到 `encv2:`；只有候选 runtime 健康且 current release state 已落盘后，才把同一 migration state 原子转为 `migration-committed`。按需发布会先从当前 release 读取未选服务镜像，避免 `.env` 被旧 tag 覆盖；如果目标机已有旧部署但还没有 `releases/current-release.env`，脚本会用服务器 `.env` 作为初始镜像基线并写入 release manifest，不需要把 `services` 留空。按需发布包含 `httpapi`、`dataworker` 或 `ragworker` 时会纳入 migration；上游签名计划必须显式给出 HttpApi/Migration/DataWorker/RagWorker/Web 完整迁移闭包，migration-only 或部分组会在容器更新前拒绝；web-only 发布可以不带 migration。服务启动后会探测 Web 首页、HTTP-only 安全头，并自动运行 `scripts/check-release-security-attestation.sh` 验收 Cloud OIDC 状态接口、Web 非 root 和 API key 密文迁移。部署完成后会写入 `releases/current-release.env`、`previous-release.env`、`staged-release.env`、`current-release.summary.md` 和 `history/`，其中 summary 会包含 release security attestation 输出。
