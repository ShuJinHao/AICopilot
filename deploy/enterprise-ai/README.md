...
- AICopilot 选择 HttpApi/DataWorker/RagWorker 时自动包含 migration；任何包含 migration 的发布必须同时选择 HttpApi/DataWorker/RagWorker/Web 完整 runtime 组，migration-only 或部分组直接拒绝。所有生产入口在 migration 前生成 PostgreSQL dump/checksum；稳定 Runner 随后停稳全部四个旧 runtime、持久化不可自动恢复的 started marker，再对完整候选 runtime 组执行 `--no-deps` 更新；PostgreSQL、RabbitMQ、Qdrant 不随应用发布重建。
...
后端容器以非 root 用户运行，RAG 文档上传必须落在 compose 声明的持久化可写卷。RAG 存储根、journal/lease/marker 对账、保留和平台边界的唯一技术正文是 [DDD 聚合根边界](../../docs/DDD聚合根边界.md)；本操作入口只消费 compose 与 `.env.example` 的部署配置，不重新定义持久化算法。
...
4. 入口通过一次 SSH 发送私有 digest-bound request；migration 请求必须包含五服务完整闭包，稳定 Runner 串行执行备份、全部旧 runtime quiesce、started marker、migration、`--no-deps` rollout、健康检查和 history。migration 只有在 compose 成功、容器真实退出码为 `0`、并出现与本次 invocation 精确绑定的成功标记后才放行 runtime；started 后失败禁止自动恢复旧 runtime并阻断重试。
...
服务器必须预先具备 Harbor pull 凭据；本机构建脚本按需执行 `docker login`，服务器 `deploy-release.sh` 本身只重写所选应用镜像 tag并执行 `docker compose pull`，不伪装成会自动登录 Harbor。全量或按需发布在 migration 前都必须生成完整 PostgreSQL dump 和 SHA-256；随后停稳四个旧 runtime、运行 `aicopilot-migration`，并在启动 HttpApi/DataWorker/RagWorker/Web 之前同时确认 compose 成功、migration 容器真实退出码为 `0`、与本次 invocation 绑定的成功标记存在，再检查模型和 Embedding API key 已全部迁移到 `encv2:`。按需发布会先从当前 release 读取未选服务镜像，避免 `.env` 被旧 tag 覆盖；如果目标机已有旧部署但还没有 `releases/current-release.env`，脚本会用服务器 `.env` 作为初始镜像基线并写入 release manifest，不需要把 `services` 留空。按需发布包含 `httpapi`、`dataworker` 或 `ragworker` 时会纳入 migration；上游签名计划必须显式给出 HttpApi/Migration/DataWorker/RagWorker/Web 完整迁移闭包，migration-only 或部分组会在容器更新前拒绝；web-only 发布可以不带 migration。服务启动后会探测 Web 首页、HTTP-only 安全头，并自动运行 `scripts/check-release-security-attestation.sh` 验收 Cloud OIDC 状态接口、Web 非 root 和 API key 密文迁移。部署完成后会写入 `releases/current-release.env`、`previous-release.env`、`staged-release.env`、`current-release.summary.md` 和 `history/`，其中 summary 会包含 release security attestation 输出。
