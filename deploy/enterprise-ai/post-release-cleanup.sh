...
Runs safe post-release cleanup after AICopilot deployment:
...
if [ -n "$RELEASE_TAG" ] && [[ ! "$RELEASE_TAG" =~ ^sha-[0-9a-f]+$ ]]; then
...
prune_local_docker_images() {
  ...
  if [ "$removed" -eq 0 ]; then
    printf -- '- No local old AICopilot application image tags found.\n'
  fi
}
