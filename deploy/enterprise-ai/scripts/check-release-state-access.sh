...
RELEASE_HISTORY_DIR="${RELEASE_HISTORY_DIR:-$RELEASES_DIR/history}"
CURRENT_RELEASE_FILE="${CURRENT_RELEASE_FILE:-$RELEASES_DIR/current-release.env}"
PREVIOUS_RELEASE_FILE="${PREVIOUS_RELEASE_FILE:-$RELEASES_DIR/previous-release.env}"
STAGED_RELEASE_FILE="${STAGED_RELEASE_FILE:-$RELEASES_DIR/staged-release.env}"
CURRENT_RELEASE_SUMMARY_FILE="${CURRENT_RELEASE_SUMMARY_FILE:-$RELEASES_DIR/current-release.summary.md}"
...
fail_release_state() {
  ...
  printf 'AICopilot release state is not accessible to standard non-root deploy user %s:%s: %s (%s)\n' \
    "$current_user" \
    "$current_group" \
    "$path" \
    "$reason" >&2
  ...
  printf 'Fix modes, for example: find %s -type d -exec chmod 755 {} + && find %s -type f -exec chmod 600 {} +\n' \
    "$RELEASES_DIR" \
    "$RELEASES_DIR" >&2
  printf 'If a root emergency path was used, restore owner/mode first, then rerun the standard non-root gate.\n' >&2
  exit 73
}
...
printf 'AICopilot release state access check passed: %s\n' "$RELEASES_DIR"
