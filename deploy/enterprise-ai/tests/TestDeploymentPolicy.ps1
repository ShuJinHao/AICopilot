...
Require-Text 'deploy/enterprise-ai/deploy-release.sh' 'wait_for_compose_service_healthy postgres 180[\s\S]*?quiesce_schema_dependent_runtimes[\s\S]*?backup_database_before_migration[\s\S]*?persist_migration_started_state[\s\S]*?MIGRATION_EXECUTED=true[\s\S]*?run_migration_and_verify' 'The final recovery dump and durable started marker must precede every server-side migration.'
Require-Text 'deploy/enterprise-ai/deploy-release.sh' 'find_unresolved_migration_state[\s\S]*?DEPLOY_MIGRATION_STATUS=migration-started[\s\S]*?unresolved migration state[\s\S]*?atomic_copy_file "\$STAGED_RELEASE_FILE" "\$CURRENT_RELEASE_FILE"[\s\S]*?commit_migration_state[\s\S]*?RELEASE_COMMITTED=true' 'The legacy path must block unresolved migrations and commit its marker only after current release state is durable.'
...
Require-Text 'deploy/enterprise-ai/runner/iiot-release-runner.sh' 'migration_is_unresolved[\s\S]*?commit_migration_state[\s\S]*?MIGRATION_STARTED=0' 'A committed migration must clear its in-memory partial state and let exit handling trust matching durable markers.'
...
Require-Text 'deploy/enterprise-ai/scripts/install-support-release.sh' 'SUPPORT_BACKUP_DIR[\s\S]*?release_lock_is_active[\s\S]*?\[ -d "\$SUPPORT_BACKUP_DIR" \][\s\S]*?exit 86[\s\S]*?release_acquire_lock' 'Support admission must reject a residual same-token backup before a stale lock can be reacquired under that token.'
