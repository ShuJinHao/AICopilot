...
- 远端送审入口：Draft PR #56，review-only total review package。
- 初始远端送审 head：`3a46fa1 aicopilot: prepare 5.5 total review package`，`simulation-rc` 已通过；后续 request-changes 修正以后，以 PR #56 当前 head 和 GitHub Checks 为准。
...
- M7 dry-run evidence 和 runner 本地/远端送审说明：`docs/enterprise-limited-pilot-dry-run-p17_2-latest.md`、`scripts/Run-EnterpriseLimitedPilotDryRunP17_2Acceptance.ps1`。
...
- M7：P17.2 fake/fixture dry-run 已刷新，覆盖 fixed-template、controlled-goal、拒绝项、emergency stop、rollback、hash-only evidence；dry-run evidence 源自本地生成，但现已随 PR #56 远端送审，PR CI 只验证 review-package scope 和 release-candidate 门禁，不把 dry-run 变成真实 Pilot。
...
- `git diff --check`、`Test-TextEncoding.ps1`：在各批次执行通过。
- PR #56 初始远端送审 head `3a46fa1` 的 `AICopilot Simulation Release Candidate / simulation-rc` 已通过；Docker simulation suites 仍按当前 CI 配置为 skipped，不声明 Docker 集成套件全量通过。
...
- 不把 PR #56 的 review-only / CI success 解释为 Pilot、GA、endpoint/token 配置批准或生产执行许可。
