...
- AICopilot migration 只有在 compose 命令成功、migration 容器真实退出码为 `0`、且容器日志存在与本次 invocation 精确绑定的成功标记时才算完成；任何生产入口都必须先生成完整 PostgreSQL dump 和 SHA-256。包含 migration 的请求必须同时选择 HttpApi、DataWorker、RagWorker、Web 完整 runtime 组，migration-only 或部分组在容器写入前拒绝；稳定 Runner 在启动 migration 前持久化 `migration-started` 且 `resumeAllowed=false`，并先停全部四个旧 runtime。从该时刻起任一失败都保持 blocked-partial，不得自动重试或用旧镜像回滚已迁移 schema，只有整轮 runtime 健康并提交 current state 后才能把 marker 转为 committed。MigrationWorkApp 只能在数据库 migration、secret 迁移和全部 seed 完成后输出成功标记，任一异常必须以非零进程码终止。
