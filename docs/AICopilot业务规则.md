...
- 审批竞争、批准后的权威/源文件漂移、checkpoint 损坏、文件提交结果未知或恢复证据不完整时不得猜测完成，也不得把 active attempt 直接标记失败；必须保留批准证明与 attempt，原子进入 `ReconciliationRequired`、写结构化审计，并等待对账或人工处置。source-bound `ApprovalResume` 元组无效时，队列死信、任务/attempt 对账状态和 `Agent.FinalizationReconciliationRequired` 审计必须同事务提交，审计失败必须回滚全部状态。旧同步 finalize 入口不再创建队列、复制文件或写终态：已完整关单时只读返回既有结果，其余状态均验证后 fail-closed，由审批事务产生的唯一 durable queue 和 Worker 负责发布。
