...
- 数据库执行策略只允许对已识别的 Npgsql transient 故障重试整笔事务。前一尝试无论在业务保存、timeline、审计或 marker 阶段失败，都必须先清空 EF 跟踪状态；下一尝试重新取得锁并 fresh-read 权威后再裁决。Worker 关单前还必须重新读取任务和 attempt，禁止用失败尝试留下的 claim 快照完成队列；未知异常不得改写成 transient、审批冲突或业务成功。
