...
- 每个任务只能有一个 FinalOutput 审批；不可变 approval proof 必须绑定任务、workspace、唯一末步、活动 attempt、最终 NodeRun、task/node fencing token、`EvidenceSetDigest`、源 manifest digest，以及每个 Artifact 的 producer、版本、路径、MIME、长度和文件 SHA-256。运行时进入 proof 封存前必须显式持久化所有已完成前置步骤和等待审批的最终步骤，审批事务再以隔离 context fresh-read 这些数据库权威状态；不得依赖后续续租、调用方内存对象或隐式保存推断步骤已完成。封存 proof 时 task 与 attempt 的同一 run lease 必须尚未过期；审批后任一权威字段或源文件字节漂移都必须返回 `agent_finalization_state_conflict`。
- `Approved` / `Rejected` 必须封存绑定 approval proof、创建时间、目标 workspace、决策、决策人、决策时间和安全 comment 的 decision proof。相同决定重复提交幂等返回既有结果；相反或竞争决定返回 `agent_approval_state_conflict`。已批准恢复在后续关单中进入 `Failed` 或 `Cancelled` 不得改写既有 decision proof；proof、source-bound 队列元组、attempt 与 fencing 仍完整时，迟到的相同批准必须继续返回既有批准结果，并由任务/队列状态另行表达下游终态，不得误报审批冲突或重建队列。`Approved` 与唯一、source-bound 的 `ApprovalResume` 队列项必须在同一数据库事务中提交，任务仍保持 `WaitingFinalApproval`；`Rejected` 必须在同一事务中终止最终步骤、最终 NodeRun、活动 attempt 和任务，不得产生恢复队列，后续稳定返回 `agent_approval_rejected`。
- Artifact 草稿编辑的事务外预检不能作为提交授权。任何版本变更在数据库提交前必须按稳定顺序取得与审批封存相同的 task、workspace、Artifact 行锁，并在锁内 fresh-read `WorkspaceReady`、未 Finalized 且不存在 FinalOutput 审批；编辑与封存竞争时只能一个提交，失败编辑必须回滚 staged file-set 并继续返回“final review 后草稿锁定”的稳定结果。
- durable Worker 只有在任务仍为 `WaitingFinalApproval`、唯一最终审批已批准、最终步骤是唯一末步、前置步骤和 Artifact binding 完整、attempt/lease/fencing 与 file-set stage 全部匹配时，才可在同一权威 checkpoint 中完成 Artifact、Workspace、最终步骤、attempt、任务、最终结果 timeline 投影和结构化成功审计。最终文件哈希或复制前后的 lease renewal 必须在同一锁内 fresh-read 并要求 task、attempt、queue 和 NodeRun 四个现有 lease 均严格晚于 renewal 时间；任一 lease 缺失或过期都只能返回 stale fence，禁止把已失效 worker 的 lease 延长复活。已有 Session 的所有消息与 Agent 投影在持久化前必须按稳定顺序取得同一 Session 行锁，并在锁内基于 `messages` 与 `message_events` 的共同最大序号重新分配；本事务新建且尚未落库的 Session 从 0 本地分配首批序号，并与 Session 一起原子提交。并发写入不得碰撞或让消息与对应事件错位。投影或审计写入失败必须回滚关单，禁止先标记 `Completed` 再 best-effort 补写。`Finalized` 只是该原子关单中的内部过渡，用户可见成功只认 `Completed` 且 `CompletedAt`、最终摘要、manifest/digest 和 final 文件集一致。
- 审批竞争、批准后的权威/源文件漂移、checkpoint 损坏、文件提交结果未知或恢复证据不完整时不得猜测完成，也不得把 active attempt 直接标记失败；必须保留批准证明与 attempt，原子进入 `ReconciliationRequired`、写结构化审计，并等待对账或人工处置。Worker 已将 `ApprovalResume` 标记为 `Started`、但最终 NodeRun 尚未 claim 时发现漂移，也必须在当前 queue/task/attempt fence 下原子写入任务与 attempt 的 `ReconciliationRequired` 及 `Agent.FinalizationReconciliationRequired` 审计，保留 `Started` 队列作为 checkpoint；即使 source approval 行已经缺失、改绑到其它 task 或不再是 `FinalOutput`，仍必须使用队列封存的 `sourceApprovalRequestId` 作为审计目标完成同一对账事务，不能因审批行本身损坏而循环重排队列。恢复扫描不得把该队列重排为普通重试，审计事务失败也不得把它覆盖为 `Failed`。source-bound `ApprovalResume` 元组在 claim 前无效时，队列死信、任务/attempt 对账状态和同名审计必须同事务提交，审计失败必须回滚全部状态。旧同步 finalize 入口不再创建队列、复制文件或写终态：已完整关单时只读返回既有结果，其余状态均验证后 fail-closed，由审批事务产生的唯一 durable queue 和 Worker 负责发布。
- 数据库执行策略只允许对已识别的 Npgsql transient 故障重试整笔事务。每笔 Agent 执行事务必须使用由强类型 options 新建并独占的 `AiGatewayDbContext`，不得复用同一 scope 中仍承载运行时 task/workspace/step/attempt 的 context；该 options symbol 必须同时匹配完全限定类型和 `Microsoft.EntityFrameworkCore` 程序集身份，同名伪程序集不得获得架构例外。前一尝试无论在业务保存、timeline、审计或 marker 阶段失败，都只能清空该隔离事务 context 的 EF 跟踪状态，禁止脱离或替换外层运行时聚合。跨 AiGateway/RAG schema 的密钥迁移同样必须让每次 execution-strategy attempt 新建两个 context、重新读取两张源表并共享一个数据库事务；secondary context 包装的异常只有在异常链明确包含 `IsTransient=true` 的 Npgsql provider 异常时才可还原给外层策略，普通包装异常和业务异常不得触发重试。下一尝试重新取得锁并 fresh-read 权威后再裁决。Worker 关单前还必须重新读取任务和 attempt，禁止用失败尝试留下的 claim 快照完成队列；未知异常不得改写成 transient、审批冲突或业务成功。
