...
- AICopilot migration 只有在 compose 命令成功、migration 容器真实退出码为 `0`、且容器日志存在与本次 invocation 精确绑定的成功标记时才算完成。包含 migration 的请求必须同时选择 HttpApi、DataWorker、RagWorker、Web 完整 runtime 组并绑定实际执行 Runner 的 SHA-256，migration-only、部分组或 Runner 字节漂移在容器写入前拒绝；稳定 Runner 必须先停全部四个旧 runtime，再生成最终完整 PostgreSQL dump/SHA-256，随后持久化 `migration-started` 且 `resumeAllowed=false` 才可启动 migration。从该时刻起任一失败都保持 blocked-partial，不得自动重试或用旧镜像回滚已迁移 schema；migration 前停止或备份失败的恢复必须复验四个旧 runtime，证明容器未 Restarting/OOM、RestartCount 稳定，并经 Web 生产端口连续验证前端根路径和匿名 Cloud OIDC 状态 API 均返回 `200`；恢复失败或不可确认统一返回 `86`。只有整轮候选 runtime 健康并提交 current state 后才能把 marker 转为 committed；started/committed 双 marker 仅在不可变 identity 精确一致时允许对账清理。MigrationWorkApp 只能在数据库 migration、secret 迁移和全部 seed 完成后输出成功标记，且 AiGateway preflight 到 migration 全程持同一 PostgreSQL advisory lock；任一异常必须以非零进程码终止。
