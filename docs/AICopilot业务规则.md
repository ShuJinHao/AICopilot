...
- 每个任务只能有一个 FinalOutput 审批；不可变 approval proof 必须绑定任务、workspace、唯一末步、活动 attempt、最终 NodeRun、task/node fencing token、`EvidenceSetDigest`、源 manifest digest，以及每个 Artifact 的 producer、版本、路径、MIME、长度和文件 SHA-256。封存 proof 时 task 与 attempt 的同一 run lease 必须尚未过期；审批后任一权威字段或源文件字节漂移都必须返回 `agent_finalization_state_conflict`。
