...
- 首次 JIT、Bootstrap 管理员收编和既有账号密码确认必须共用同一 Identity 事务内 binding invariant guard；一次取得并按稳定顺序锁定 `(provider, tenantId, externalUserId)`、规范化本地用户名，以及已知或本次预分配的 `(userId, provider)`，随后重新读取用户与绑定，禁止用加锁前结果作绑定决定。完全相同的既有绑定幂等复用，任一侧指向其他身份则稳定拒绝且不得覆盖。
