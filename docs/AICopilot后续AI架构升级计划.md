...
> 状态：架构评审稿 v2.1，供 Kimi、人工架构评审和后续实施拆解使用
>
> 日期：2026-07-17
>
> 适用项目：`AICopilot`
...
>
> 核心方向：一套节点契约、两种生命周期编排；运行时地基先行；Skill 后置物理退役
...
本文将前期讨论、AICopilot 源码核验、六个开源 Agent 框架源码核验、Kimi 两轮评审及补充复核意见，整理成一份完整的后续架构计划。
...
P0 目标契约冻结
→ P1 运行时地基 R1–R7
→ P2 共享节点执行与 Evidence 平面
→ P3 Plan v2 单轨切换并物理退役 Skill
→ P4 有限 DAG
→ P5 受控 Agent Node
→ P6 完整 IntentRegistry 与唯一 PlanCompiler
...
- P3 与 Skill 一起物理删除旧实现、死参数和依赖旧路径的测试。
- P6 建设新的、受 Plan v2 和 Node Contract 约束的唯一 `PlanCompiler`。
...
### R1：PostgreSQL 原子 Claim 与 Fencing

目标：

- 多 Worker 同时领取时只有一个有效执行者。
...
- claim 在数据库事务中完成。
- 每次 claim 生成递增 fencing token。
- QueueItem、RunAttempt、NodeRun 持有有效 token 或其关联。
- 权威写入必须带当前 token 条件。
...
模型端点已有 Scheduler 继续负责模型 RPM、TPM 和 endpoint 并发；Workflow 只做上层配额与预约，不复制第二套模型端点调度器。
...
2. 过期 fencing token 无法写 checkpoint、Evidence 和完成状态。
...
10. 所有测试使用真实数据库或明确的单元边界，不用 sleep 伪造并发正确性。

---

## 13. P2：共享节点执行与 Evidence 平面
...
在删除 Skill 前，对五个内置 Skill 建立 before/after 矩阵：
...
- 五个内置 Skill before/after 权限矩阵不扩大。
- 新任务只执行 Plan v2。
...
## 17. P6：完整 IntentRegistry 与唯一 PlanCompiler

P6 在 Node、Evidence、Runtime 和 Skill 退役稳定后实施。
...
新 PlanCompiler 取代旧 DynamicPlanner 的影子路径。
...
| 阶段 | 前置 | 主要交付 | 退出门 |
|---|---|---|---|
| P0 契约冻结 | 当前源码事实已核验 | 现有 IntentRouting inventory/adapter、版本化 allowlist、Plan v2、Node Contract、Cloud/Governed 双读路径、Evidence v1、ExecutionSnapshot、Skill inventory | Schema/digest 稳定，职责迁移完整，预测节点稳定拒绝 |
| P1 运行时地基 | P0 | R1–R7 | 多 Worker、kill/recovery、fencing、取消、快照、幂等通过 |
| P2 共享执行平面 | P1 | Node Executor、Evidence Normalizer、CloudRead/GovernedDataRead 双执行器、双编排接入 | Chat/Durable 语义一致，无上下文污染和跨路径 fallback |
| P3 单轨切换 | P2 | Plan v2 上线、Skill/DynamicPlanner/死字段物理删除 | 权限不扩大，活动系统零旧消费者 |
| P4 有限 DAG | P3 | DependsOn、合流、并行 2–4 | 并发、失败和恢复稳定 |
| P5 受控 Agent Node | P4 | 独立上下文、深度 1、typed result | 无递归、越权和预算失控 |
| P6 注册与规划 | P5 | IntentRegistry、唯一 PlanCompiler | 多意图稳定、unknown fail-closed |
| P7 产品与运营 | P6 | UI、Eval、监控、当前 typed GET 数据健康评估 | 事实一致、可观测、用户语义清楚，健康结果保持 DerivedFact |
| 独立 DP 计划 | 另行授权 | 时序合同、异常模型、预测服务 | 真实数据和跨项目验收 |
...
2. 运行中强制 kill Worker。
3. 新 Worker 从 checkpoint 继续。
4. 已成功 Node 不重复产生有效结果。
5. 一个 Node timeout 并按分类重试。
6. 一个 optional Node 失败并明确显示证据缺失。
7. required Node 失败阻止综合。
8. 审批暂停后重启仍使用原 Evidence。
9. 主 Agent 只消费已授权 EvidenceSet。
10. 图表和报告复用同一 `EvidenceSetDigest`。
11. Token、成本、版本和 correlation 信息完整。
12. 用户在 Chat 中追问同一问题时，复用该任务 `EvidenceSetDigest`，与报告事实一致。
13. 用户改变时间范围后重新查询，不复用旧结论冒充新证据。
14. 原型中的正式设备/日志/产能只走 `CloudReadNode → QuerySemanticAsync → typed GET`。
15. 模拟 Cloud 失败、空集和超时，验证不会创建或调度 `GovernedDataReadNode`。
16. 单独使用一个明确的 `intentClass=GovernedExploration` 请求验证治理型二级读取，Evidence 必须标记真实 Provider，且不能进入 Cloud-only 分支。
17. 提交预测诉求或 `PredictionReadNode`，验证只返回 capability gap 且无执行器调用。
