...
- P4 在同一 Compiler 内增加固定、受限、可测试的 `DagV1` 模板，与 Validator/Runtime 同批启用。
- P6 再接入完整 IntentRegistry 和 Registry-driven 复杂多意图 DAG 组合；不是到 P6 才首次建设 Compiler 或首次生成 DAG。
...
- `PluginSelectionMode=BuiltInOnly + SelectedPluginIds=[]` 表示禁止可选 Plugin，只允许服务端内建能力继续按 Intent、权限和 Guard 求交集。
- `PluginSelectionMode=ExplicitAllowlist + SelectedPluginIds=[ids]` 表示允许内建 Provider 加这些 Plugin；未列出的 Plugin 一律不可用，空列表属于无效 mode/list 组合。
- `CapabilitySelectionMode=ExplicitAllowlist + RequestedCapabilityCodes=[]` 表示没有可执行业务能力，只能返回普通回答或 capability gap。
- `CapabilitySelectionMode=InferredFromGoal` 只能采用已接受 Intent 直接要求的 canonical capability，不能展开成全目录；Compiler 必须把推导结果写回 Plan v2 的 `requestedCapabilityCodes`，可执行计划中该数组不得为空。
...
- `AI-PERSIST-01d / AI-SEC-047` 作为 P1 并行工作项和 P1 退出硬门；未关单时 P1 不得完成，P2/P3 自然不得开始。
...
| 领取粒度未分层 | 当前 QueueItem 按 task 领取，尚无独立 NodeRun claim/fencing 协议 | DAG 并行时无法证明节点只有一个有效执行者 |
...
| Step 结构化 JSON 截断 | `AgentStep.InputJson/OutputJson` 最大 16000 字符且超限取前缀 | 可能损坏输入语义，也不能承担完整 Evidence 和大型结果 |
| Plan 静默截断 | `AgentTask.NormalizeRequired(planJson, ..., 32000)` 对超长字符串直接取前缀 | Plan 可能变成不可解析或 digest 不对应的 JSON |
| 模型配额仅进程内 | `InMemoryModelEndpointPoolScheduler` 用 `ConcurrentDictionary` 保存 endpoint/user/role/tenant 窗口 | 多 HttpApi/DataWorker 实例可各自放行并突破全局限额 |
| Artifact 文件集对账未闭环 | `AI-PERSIST-01d / AI-SEC-047` 仍为 Partial | 大 Evidence 不能仅靠路径引用宣称可恢复 |
...
目标 catalog 只定义以下有限 NodeKind，并按阶段启用：
...
- P0–P2：冻结全部名称，但生产只允许确定性线性节点；`ArtifactGenerationNode` 在 01d 关单前禁用，`JoinNode`、`AgentReasoningNode` 禁用。
- P3：允许已完成 Guard/Evidence/执行器验收的线性节点，仍禁用 `JoinNode` 和 `AgentReasoningNode`。
- P4：`DagV1` 同批启用 `JoinNode`。
- P5：受控协议验收后才启用 `AgentReasoningNode`。
- 任一阶段的 inactive NodeKind 必须由 Compiler、Validator、Runtime 三层稳定拒绝。
...
- 只有来自已加载授权 roster、当前会话上下文或用户明确选择的稳定 ID 才能进入可执行 Plan；若自然语言资源名无法在不执行 Tool/Cloud 查询的前提下唯一解析，PlanDraft 返回 `resource_resolution_required` 并要求用户选择，不猜 ID、不在确认前查询。
...
`PlanDraft` 可以携带上述 Plan v2 的候选副本和 `planDigest` 供用户确认，但其 lifecycle 仍是不可执行草案。确认时必须重新读取能力/权限/Policy 元数据并验证候选 ExecutionSnapshot；只有完整 digest 与用户看到并确认的 digest 一致，才能 seal 为 `ExecutablePlan`。若确认时发生任何 digest-affecting 漂移，必须生成新 PlanDraft 并重新确认，不能在确认动作中静默补字段或换版本。
...
2. `PlanContract:MaxCanonicalBytes` 在 Plan v2 contract 中固定为 262,144 bytes；超限返回稳定 `plan_payload_too_large`，不写入数据库。修改该值必须升级 contract/policy version 并进入 snapshot/digest，不能以运行时配置静默改变旧计划语义。
...
6. `AgentStep.InputJson/OutputJson` 同批删除 16,000 字符 substring：小型 typed input 显式校验后完整保存，Node 大输出只写 Evidence/Artifact 引用；禁止通过放大上限继续承载恢复权威。
...
8. `AI-PERSIST-01d / AI-SEC-047` 已登记为 P1 并行工作项和退出硬门，未完成前的 inline-only 限制已有自动化门禁。
...
∩ 用户权限
∩ ProviderScope(PluginSelectionMode, SelectedPluginIds)
∩ RequestedCapabilityCodes
...
- `ProviderScope(BuiltInOnly, []) = 当前启用的内建 Provider`。
- `ProviderScope(ExplicitAllowlist, ids) = 当前启用的内建 Provider ∪ ids 对应且当前启用的 Plugin Provider`。
- `RequestedCapabilityCodes` 在可执行 Plan v2 中必须是 Compiler 已规范化的非空 canonical 集合；普通 Chat 或纯 capability-gap PlanDraft 不生成空 ExecutablePlan。
...
请求入口可以省略用户未操作的高级选择，但进入 PlanCompiler 前必须规范化：

- 未显式选择 Plugin → `PluginSelectionMode=BuiltInOnly`、`SelectedPluginIds=[]`。
- 显式勾选 Plugin → `PluginSelectionMode=ExplicitAllowlist`，列表必须非空且只含当前用户可见稳定 ID。
- 未显式选择业务能力 → `CapabilitySelectionMode=InferredFromGoal`，由 accepted Intent 产生最小 capability 集并写回 Plan v2；无法推导时只返回 capability gap。
- 显式选择业务能力 → `CapabilitySelectionMode=ExplicitAllowlist`；空列表表示不允许执行任何业务能力。
- 内建 `CloudReadNode`、Policy 和确定性计算不是“隐形 Plugin”，但仍必须落在 accepted Intent、用户权限、Node 请求和 Guard 的交集中。
- API/Plan/Runtime 看到 null、未知 mode、重复 ID、禁用 ID 或 mode/list 不匹配时必须拒绝，不能猜测成默认全选。
...
- `InlineCanonicalJson` 的单份 payload 上限在 Evidence v1 中固定为 65,536 UTF-8 bytes；超限返回 `evidence_payload_too_large`，不得截断或只留下摘要后把 Node 标记成功。修改上限必须升级 Evidence contract/policy version。
...
| 对象 | 允许迁移 | 关键条件 |
|---|---|---|
| AgentTask | Draft → WaitingPlanApproval → PlanApproved → Queued → Running；Running ↔ WaitingApproval；Running → ReconciliationRequired；ReconciliationRequired → Running/Succeeded/Failed/Cancelled；Running → Succeeded/Failed/Cancelled | `Succeeded` 要求全部 required Node 已成功且不存在 OutcomeUnknown；等待/对账状态不得输出最终成功 |
| QueueItem | Queued → Claimed → Started → Succeeded/Failed/Cancelled；Claimed → Queued；Started → Claimed | `Claimed → Queued` 仅限执行开始前 lease 过期；`Started → Claimed` 仅由恢复协调器在旧 task lease 过期并先收口活动 Node 后生成新 task fencing |
| RunAttempt | Created → Running；Running ↔ WaitingApproval；Running → ReconciliationRequired；ReconciliationRequired → Running/Succeeded/Failed/Cancelled；Running → Succeeded/Failed/Cancelled | 一次只允许一个 active attempt；恢复同一 attempt 必须生成新 task fencing，用户显式 Retry 才创建新的 attemptNo |

未列出的迁移全部非法；Succeeded/Failed/Cancelled 终态不得回流。QueueItem、RunAttempt 和 AgentTask 只能由对应 coordinator 在同一权威事务中根据 NodeRun 聚合事实推进，不能由 Controller/Worker 任意直接跳转。
...
4. 权威事实证明副作用未发生时，迁移为 `Failed`；只有 retry policy 明确允许且生成新的 node-attempt generation/幂等代时，才可迁移为 `Runnable`，不得偷换成用户级新 RunAttempt。
...
- P2 的同一生产 PlanCompiler 增加固定 `DagV1` skeleton，只允许本节列出的合流和并行策略，不创建第二套 DAG Planner。
- 稳定 `NodeId`。
- `DependsOn`。
- 无环校验。
- 并行度 2–4。
...
P2 的生产 PlanCompiler 已在 P4 支持固定受限 `DagV1`；P6 在同一边界接入完整 Registry，并扩展 Registry-driven 的复杂多意图 DAG 组合。旧 DynamicPlanner 已在 P3 前退出，不在本阶段再次迁移。
...
| P4 有限 DAG | P3 | 同一 Compiler 的固定 DagV1 skeleton、DependsOn、合流、并行 2–4 | 新 digest/确认、并发、失败和恢复稳定 |
| P5 受控 Agent Node | P4 | 独立上下文、深度 1、typed result | 无递归、越权和预算失控 |
| P6 注册与规划泛化 | P5 | 完整 IntentRegistry、同一 PlanCompiler 的 Registry-driven 复杂多意图 DAG 泛化 | 单一 Compiler、多意图稳定、unknown fail-closed |
...
- P2 起只有一个生产 PlanCompiler；不得重新注册 DynamicPlanner、shadow compiler 或手写 PlanJson 生产入口。
- P0–P3 只允许 `LinearV1`，P4 起 `DagV1` 必须显式 profile；两者不得由 Runtime 猜测或静默互转。
- 零 Skill 活动消费者。
- 无新增伪聚合根。
- Queue/NodeRun/Evidence/Quota 使用明确 Store，schema/migration 只归 `AiGatewayDbContext`。
- Task claim、Node claim、checkpoint 和 outcome reconciliation 各只有一个 transaction owner。
- 不存在 v1/v2 双执行器。
...
- IntentCandidate Schema：canonical provider/resource ID、typed filter/time range、required provenance、KnownButUnavailable 和 capability gap。
- 未解析或多义资源名只能形成 `resource_resolution_required`，不能进入 ExecutablePlan。
- 原始 Query/Reasoning/Prompt 不进入 IntentCandidate、Plan、Evidence 或日志。
...
- `LinearV1/DagV1` profile、依赖约束和 profile 升级重新确认。
- PlanJson 与 AgentStep typed JSON 的 canonical UTF-8 byte limit、边界值、超限错误、round-trip 和 digest；禁止 substring。
- Plugin/Capability selection mode 的 null/empty/built-in/explicit/inferred 语义。
- Graph 无环校验。
- 权限交集。
...
- 状态迁移表、终态不可回流、OutcomeUnknown 对账决议。
- Inline Evidence 65,536-byte 边界和 ArtifactReference 完整性。

### 22.3 Application / Workflow

- PlanDraft 不执行 Tool。
- P2 最小 Compiler 生成确定性 `LinearV1` PlanDraft；用户确认后生成 Plan v2。
- 同一输入/catalog/version 的安全骨架稳定，模型不能改变 NodeKind、权限、资源范围、审批或拓扑。
- 缺能力、显式空 allowlist、KnownButUnavailable 和 Unknown 只产生 capability gap，不产生空可执行计划。
- Chat 四分支语义不回退。
...
- 节点超时和总超时。
- Evidence-only 综合。
- 全部 active Skill/用户/角色/资源 before/after 允许等价、拒绝保持和明确退役差分。

### 22.4 Persistence

- PostgreSQL Task claim 与 Node claim 独立原子竞争。
- task/node fencing token 独立且不可互换。
- checkpoint 原子性。
- stale worker reject。
- crash/restart。
- migration。
...
- marker/Artifact 对账。
- PostgreSQL 模型 quota reservation 的 acquire/settle/expire/reconcile。
- OutcomeUnknown fenced reconciliation 与人工决议审计。
...
- Queue lease 过期。
- 旧 fencing token 提交。
- Task token 正确但 Node token 过期，以及 Node token 正确但 Task token 过期。
- Artifact 文件完成但数据库结果未知。
- Artifact payload 缺失、hash/size/media type 不一致和 final copy 中断。
- 模型预约后 Worker kill、调用完成但 settle ACK 丢失、多个进程同时抢最后一个并发槽。
- OutcomeUnknown 权威成功、确认未发生、冲突回执和 deadline 后仍未知。
- 用户取消与宿主关闭。
...
- Cloud Evidence 保存 URL/route/host 而不是 logical provider operation code。
...
- LLM 推断伪装事实。
- 用户选择 Plugin 试图扩大权限。
- null/empty selection 被解释成全部 Plugin 或全部 capability。
- 原始 Intent Query、Reasoning、完整用户 Prompt 或模型思维过程进入持久化/日志。
- 超限 Plan/Evidence 被截断后仍标记成功。
- Plan 中插入 Cloud 写语义。

### 22.7 Frontend

- Skill UI 物理退出。
- SelectedPluginIds/RequestedCapabilityCodes 及其 selection mode 真实生效，空值不显示为“全部”。
- 会话隔离。
- Plan digest 变化重新确认。
- 运行详情脱敏。
...
- B0 baseSha/tree、测试项目、Analyzer、required workflow、发现/执行/Skip 状态。
- 当前 `PlanJson` 长度、JSON 可解析性、32,000 字符边界命中、schema version 和 digest 可重建性。
- QueueItem、RunAttempt、AgentTask lease/token/status 的现有非终态组合。
- 当前进程内模型 Scheduler 的所有调用方和多实例部署拓扑。
- ArtifactWorkspace 未完成 journal/marker、文件集状态和 `AI-PERSIST-01d` 影响路径。

P3 前必须再次盘点：

- 全部 SkillDefinition 行、内置定义、Plugin/MCP 关联和真实消费者。
- 未终态 AgentTask。
- PlanJson schema version。
- topology profile、selection mode 和 plan digest。
...
4. 确认 P2 唯一生产 Compiler 已能覆盖全部保留能力并只生成 `LinearV1`。
5. 执行 Plan v2/Skill 退役 migration。
6. 启动唯一新版本。
7. 验证 fresh database 和升级数据库。
8. 验证旧 Skill API 为物理不存在。
9. 验证新请求只产生完整、可 round-trip、digest 一致的 `LinearV1` Plan v2。
...
- 对切换期间任务按 plan digest、schema version 和 topology profile 对账。
- 不通过恢复 Skill 空壳临时换绿。
...
| 风险 | 控制 |
|---|---|
| 测试治理文档与实施分支分叉 | B0 唯一 clean baseSha/tree 和全量 inventory 重建 |
| Skill 删除后权限扩大或保留能力丢失 | 全 active inventory 的允许等价、拒绝保持、退役显式差分 |
...
| P3 删除旧 Planner 后没有 Plan 生产者 | P2 先交付唯一最小 LinearV1 PlanCompiler |
| DAG 字段提前进入线性 Runtime | topologyProfile 三层校验，P4 才启用 DagV1 |
| null/empty 能力选择变成全部允许 | 强制 selection mode、非 null 数组和正反例 |
| Plan 漂移 | 确认时不可变 snapshot 和 digest |
| Plan/Evidence 静默截断 | canonical byte limit、显式错误、round-trip/digest 门禁 |
| Worker 重复执行 | Task/Node 两级原子 claim、独立 fencing、幂等和 checkpoint |
| 多实例模型配额失效 | PostgreSQL quota reservation；内存 Scheduler 仅本地提示 |
...
| Artifact 重复或丢失 | AI-PERSIST-01d 作为 P1 硬门，完成前禁止 ArtifactReference |
| OutcomeUnknown 被盲重放或永久假终态 | fenced reconciliation、deadline、人工决议和下游阻断 |
...
- B0 无法形成唯一 clean 实施基线，或测试治理状态仍与源码冲突。
- 权限差分显示新能力扩大、保留能力意外丢失或旧拒绝失效。
- P2 尚无唯一生产 LinearV1 PlanCompiler，却准备删除 Skill/DynamicPlanner。
- P4 前 Plan/Validator/Runtime 接受分叉、合流或 `DagV1`。
- PlanJson/Evidence 超限时仍被截断、部分保存或标记成功。
- 真实 PostgreSQL 多 Worker 出现双执行者。
- Task/Node fencing token 可互换，或任一 stale token 可以提交。
- 多实例模型调用突破 tenant/user/model/endpoint 配额。
- Worker kill 后无法确定恢复点。
- 取消仍被记录为普通失败。
- Evidence 可以跨用户或跨任务读取。
- Plan snapshot 漂移不能检测。
- `AI-PERSIST-01d` 未关单却启用 ArtifactWrite/ArtifactReference，或 Artifact outcome unknown 被自动重放。
- OutcomeUnknown 可以直接重试、猜测失败/成功或绕过对账进入下游。
...
2. 唯一 PlanCompiler 从 typed IntentCandidate 生成受限 PlanDraft；P3 前为 `LinearV1`，P4 后可显式生成 `DagV1`。
3. Plan 模式只生成 PlanDraft，不执行真实能力。
4. 用户确认后冻结 Plan v2、Node Contract 和 ExecutionSnapshot。
5. Durable Orchestrator 原子领取任务，Node coordinator 再独立领取 runnable Node。
6. Cloud、RAG、文件和确定性计算 Node 并行收集证据。
7. 每个 Node 完成后以双 fencing 提交 NodeRun + Evidence checkpoint。
8. Worker 中断后从最后 checkpoint 恢复；未知副作用进入 ReconciliationRequired 而非盲重放。
9. 主 Agent 只消费已授权 EvidenceSet。
10. 回答、图表和 PPT 绑定同一 `EvidenceSetDigest`。
11. 每个结论区分事实、确定性派生、预测、AI 推断和建议。
12. 用户在 Chat 中追问时复用同一 EvidenceSet，并保持事实一致。
13. 用户改变时间范围后重新查询。
14. 全过程受用户权限、Tool Guard、数据范围、审批、分布式预算和 Cloud 只读约束。
...
- B0 和 P0–P7 各阶段退出门全部通过。
- 实施 HEAD 与测试治理基线一致，新增测试已进入目标分层、Analyzer 和 required workflow。
- Skill、DynamicPlanner 影子路径和死参数物理删除。
- 当前正式规则和专题契约同步更新。
- DDD 聚合根白名单同步更新。
- 真实 PostgreSQL、多 Worker、故障注入和安全负例通过。
- Chat 与 Durable 共享 Node/Evidence 语义。
- 唯一 PlanCompiler 已在 P2 投产并于 P6 泛化；不存在 Compiler 空窗或第二套 Compiler。
- Plan v2 单轨运行，LinearV1/DagV1 profile 不混淆。
- Task/Node 两级 claim、独立 fencing、状态迁移和 OutcomeUnknown 对账已通过真实故障测试。
- PostgreSQL 分布式模型配额在多实例下生效。
- `AI-PERSIST-01d / AI-SEC-047` 已关单，大 Evidence/ArtifactReference 可验证恢复。
- PlanJson/Evidence 不存在静默截断。
...
v2.2 对两轮独立评审的全部硬缺口作出明确裁决：

1. 最小生产 PlanCompiler 前移到 P2；P6 只做同一 Compiler 的 Registry/DAG 泛化。
2. P0–P3 固定 `LinearV1`，P4 才显式升级 `DagV1` 并重新确认。
...
10. Skill 退役从单向 `新 ⊆ 旧` 升级为保留能力等价、退役能力显式为空、旧拒绝保持，覆盖全部 active inventory。
11. Plugin/Capability 选择补齐 mode、null/empty 和 built-in 语义；空数组不再可能被解释为全部允许。
12. Cloud Evidence 使用逻辑 `providerOperationCode`，不再保存实际 endpoint/path。
13. 当前 Cloud 数据健康评估唯一归属 P7；独立预测路线从 DP1 开始，并补齐 P7 与 DP1–DP3 验收门。

实施优先级固定为：

```text
B0 基线对账
→ P0 契约/PlanJson/LinearV1/影子 DI
→ P1 两级 claim + 状态机 + OutcomeUnknown + 分布式配额 + AI-PERSIST-01d
→ P2 共享执行/Evidence + 最小生产 Compiler
→ P3 单轨切换/Skill 物理退役
→ P4–P7 能力扩展和产品收口
```

此后如源码事实未变化，不再以泛化评审为由重开上述裁决；实施中出现停止条件时，按对应阶段回到事实和测试修正，不绕过门禁继续推进。
...
| PlanJson 持久化/长度行为 | `src/core/AICopilot.Core.AiGateway/Aggregates/AgentTasks/AgentTask.cs` |
...
| Queue Store | `src/infrastructure/AICopilot.EntityFrameworkCore/Repository/AgentTaskRunStores.cs` |
...
| 模型端点内存调度 | `src/infrastructure/AICopilot.AiRuntime/ModelEndpointPoolScheduler.cs` |
...
| 议题 | 旧计划/早期意见 | 本计划最终口径 |
|---|---|---|
| 实施基线 | 测试治理文档可直接视为当前事实 | B0 先证明治理 SHA 已进入实施 HEAD 或重建等价基线 |
| Skill 顺序 | 先删 Skill | P0 契约、P1 地基、P2 双链+最小 Compiler、P3 再删 |
...
| IntentRegistry | Phase 0 全量 | P0 最小契约，P2 最小 Compiler，P6 完整 Registry/Compiler 泛化 |
| Evidence | 所有链统一持久化 | Chat 默认临时，Durable 持久化 |
| 大 Evidence | 直接引用 ArtifactWorkspace | 01d 关单前只允许 65,536-byte inline；关单后才允许受控 ArtifactReference |
| 主 Agent 输入 | EvidenceDigest | 已授权 EvidenceSet，digest 校验完整性 |
| Tool 选择 | PreferredToolCodes | SelectionMode + 非 null SelectedPluginIds/RequestedCapabilityCodes 硬上限 |
| DynamicPlanner | 未来复用 | 物理删除影子路径，P2 新建最小唯一 Compiler，P6 原位泛化 |
| Plan 拓扑 | v2 一开始暴露 DAG | P0–P3 固定 LinearV1，P4 才以新 digest 启用 DagV1 |
| 领取粒度 | task/node 含糊共用 | QueueItem/Attempt 任务 claim 与 NodeRun claim 两级独立 fencing |
| PlanJson 超限 | 32,000 字符静默截断 | 262,144 UTF-8 bytes 显式失败，完整 canonical round-trip/digest |
| 模型配额 | 进程内 Scheduler 统一负责 | PostgreSQL 预约为跨实例权威，内存 Scheduler 仅本地提示 |
| OutcomeUnknown | 状态名和“不重试” | fenced 对账、deadline、人工决议、下游阻断的完整收口协议 |
...
| DP0 归属 | 与 P7 当前健康评估重复 | 当前健康评估只归 P7，独立预测路线从 DP1 开始 |
...
| Task Fencing Token | 约束 QueueItem/RunAttempt/AgentTask active attempt 的任务级令牌 |
| Node Fencing Token | 约束单个 NodeRun claim/checkpoint 的节点级令牌，不能替代 Task token |
| LinearV1 | P0–P3 的线性拓扑 profile，只允许首节点为空依赖、其余依赖紧邻前一节点 |
| DagV1 | P4 起显式启用的有限 DAG profile，必须重新生成 digest 并确认 |
...
| OutcomeUnknown | 外部副作用可能已发生但结果无法确认、必须阻断重放并进入 fenced 对账的 Node 状态 |
| ReconciliationRequired | 任一 required Node 为 OutcomeUnknown 时的任务/attempt 阻断状态 |
...
1. 先以 B0 建立唯一可信实施基线，不在分叉的测试事实上继续叠加架构。
2. 用统一 Node Contract 表达企业任务，用 Chat 和 Durable 两种编排满足短回答与长任务。
3. P2 先交付唯一最小 `LinearV1` PlanCompiler，P3 才允许删除 Skill/DynamicPlanner；P4 再显式升级 `DagV1`。
4. 用 Evidence 统一事实、恢复、引用和产物，并以 `AI-PERSIST-01d` 保护大结果文件集。
5. 用 Task/Node 两级原子 claim、独立 fencing、checkpoint、状态机、OutcomeUnknown 对账和分布式配额保证长工作流稳定。
6. 在替代约束和全量能力/拒绝差分完成后物理退役 Skill，避免重复概念和能力回归。
7. 保留 Cloud 正式语义读取和治理型自由探索两条互斥只读路径，不允许失败自动跨路径回退。
8. 继承现有 IntentRouting 资产并逐阶段收敛，P6 只做完整 Registry 和同一 Compiler 的泛化。
9. 设备预测以真实数据质量和专用模型为核心；当前健康评估归 P7，独立路线从 DP1 开始，未来 AICopilot 只读消费和解释。

开源 Agent 框架证明了独立子运行、工具过滤、上下文 fork、前后台统一控制和受控委派是可行的；AICopilot 必须在此基础上补上企业耐久性、Evidence 授权和工业安全边界。

这就是 AICopilot 后续 AI 架构升级的最终实施总计划。
