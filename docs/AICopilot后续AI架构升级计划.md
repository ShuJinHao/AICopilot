...
> 状态：架构评审稿 v2.0，供 Kimi、人工架构评审和后续实施拆解使用
>
> 日期：2026-07-17
>
> 适用项目：`AICopilot`
...
本文将前期讨论、AICopilot 源码核验、六个开源 Agent 框架源码核验、Kimi 两轮评审及复核意见，整理成一份完整的后续架构计划。
...
- 预留 Prediction Evidence 和预测服务调用契约。
...
- 当前八个 Cloud AiRead typed GET 继续作为正式唯一路径。
...
- `ModelPrediction` 必须带模型版本、置信度、预测窗口、生成时间和有效期。
...
| 阶段 | 前置 | 主要交付 | 退出门 |
|---|---|---|---|
| P0 契约冻结 | 当前源码事实已核验 | Plan v2、Node Contract、Evidence v1、ExecutionSnapshot、最小 Intent、Skill inventory | Schema/digest 稳定，职责迁移完整 |
| P1 运行时地基 | P0 | R1–R7 | 多 Worker、kill/recovery、fencing、取消、快照、幂等通过 |
| P2 共享执行平面 | P1 | Node Executor、Evidence Normalizer、双编排接入 | Chat/Durable 语义一致，无上下文污染 |
...
| P7 产品与运营 | P6 | UI、Eval、监控、当前数据健康评估 | 事实一致、可观测、用户语义清楚 |
...
| 设备预测 | 主计划 Phase 7 | 当前健康评估留主计划，时序/预测另立跨项目计划 |
...
6. 最后再增加有限 DAG、受控 Agent 和完整 IntentRegistry。
7. 设备预测以真实数据质量和专用模型为核心，AICopilot 只读消费和解释。
