...
- 本地多服务镜像构建必须为每个 .NET 入口点使用统一输出目录下的服务私有 `--artifacts-path`，不得把 SDK artifacts 放进 macOS 临时 detached source worktree；同一 SHA 只共享源码快照，不共享 `bin/obj`，也不得混用 `/private/var` 与 `/var` 路径别名。构建顺序不得影响 ProjectReference 闭包、PackageReference 版本解析或编译输入。
