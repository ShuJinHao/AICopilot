...
- 日常入口内部生成 invocation、远端 SHA、服务闭包、OCI digest、Runner SHA-256 和 request digest，操作者不再手工复制 plan path/SHA/digest。选择 HttpApi/DataWorker/RagWorker 时必须自动扩展为 HttpApi/Migration/DataWorker/RagWorker/Web 完整闭包；AICopilot Runner 必须在容器读取或写入前用签名请求核对自身正在执行的字节。请求 digest 是误操作和传输完整性门禁，不是抵御已控制部署账号攻击者的密码学边界。
- migration 放行必须同时满足 compose 命令成功、`aicopilot-migration` 容器真实 `State.ExitCode=0`，以及容器日志存在 `aicopilot_migration_result=success` 且 `invocation_id` 与本次请求完全一致。所有生产入口在 migration 前必须生成完整 PostgreSQL dump/checksum；包含 migration 的请求必须选择 HttpApi/Migration/DataWorker/RagWorker/Web 完整组，稳定 Runner 还必须先停稳全部四个旧 runtime并持久化不可自动恢复的 started marker。应用异常但 compose 返回 `0`、标记缺失/错绑或日志不可读均视为 blocked-partial；四个候选 runtime 和 current release state 均不得更新，旧 runtime 不得在未知迁移结果上自动拉起。
...
- 仅 migration 尚未启动的可逆失败允许恢复并复验 support、compose、release state、PostgreSQL、RabbitMQ、Qdrant、HttpApi、DataWorker、RagWorker 和 Web；进程 Running、非 Restarting、非 OOM、同一容器 RestartCount 稳定，且已有 Health 为 healthy 才能称容器恢复。对不声明 Docker healthcheck 的 Web/HttpApi，还必须经 Web 生产端口连续验证 `/` 与匿名 `/api/identity/cloud-oidc/status` 均为 `200`。`compose up` 成功但任一 runtime 未稳定健康、HTTP 链路不可用、恢复命令失败或结果不可验证时必须返回 unsafe-partial `86`、`resumeAllowed=false`，不得写 `ROLLBACK_STATUS=completed`。持久化 started marker 后的任何失败都保持 blocked-partial，禁止脚本猜测回滚数据库或把旧 runtime拉回已迁移 schema；若崩溃只遗留 started 与其精确 committed counterpart，后续只能核对不可变 identity 后清理 started，不能把不匹配的双 marker 当作成功。
