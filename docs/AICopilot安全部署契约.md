...
- migration 放行必须同时满足 compose 命令成功、`aicopilot-migration` 容器真实 `State.ExitCode=0`，以及容器日志存在 `aicopilot_migration_result=success` 且 `invocation_id` 与本次请求完全一致。所有生产入口在 migration 前必须生成完整 PostgreSQL dump/checksum；稳定 Runner 还必须先持久化不可自动恢复的 started marker并停选中的旧 runtime。应用异常但 compose 返回 `0`、标记缺失/错绑或日志不可读均视为 blocked-partial；四个候选 runtime 和 current release state 均不得更新，旧 runtime 不得在未知迁移结果上自动拉起。
