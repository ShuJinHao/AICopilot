...
- 2026-07-14 平台可行性审计确认当前三个远端均为 `owner.type=User` 的公开个人仓库。GitHub Enterprise Cloud 的 source-repository/workflow 绑定型 ruleset workflow 可在 organization/enterprise 层配置；普通 organization 或 GitHub Team ruleset 不得在未核实目标 plan 和 feature availability 时冒充具备该能力。user-owned repository 又没有 team required reviewer；个人仓库虽可要求具备 write 权限的真实用户审批、也可把 status source 绑定到 GitHub App，但通用 GitHub Actions App 仍允许候选 workflow 产出同源 check，不能替代 base-owned provenance。专用 attestor App 必须已安装到仓库、具有 expected-source 选择所需的 `statuses:write`、近期提交过 check run，并与既有 required status check 关联；本协议要求它创建绑定原始 PR-head SHA 的 check run，因此还必须具有 `checks:write`。如果未来只发布 commit status 而不创建 check run，必须先版本化修改本协议的 check-run 证据口径，不能把 status 冒充 check run。满足平台选择条件仍不等于候选不可调用，后者必须另行审核。候选分支新增的 `CODEOWNERS` 也不能保护自身首次 PR，因为 GitHub 读取 base branch 上的版本；`H0` 首次接入必须依赖预先存在的独立 reviewer/protection，而不是候选 CODEOWNERS 自证。实施前必须重新复核 [GitHub Enterprise Cloud required workflow 规则](https://docs.github.com/en/enterprise-cloud@latest/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/available-rules-for-rulesets#require-workflows-to-pass-before-merging)、[required status source 规则](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/available-rules-for-rulesets#require-status-checks-to-pass-before-merging)、[Checks API 权限](https://docs.github.com/en/rest/guides/using-the-rest-api-to-interact-with-checks) 与 [CODEOWNERS 规则](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners)，不能把本次时点结论永久硬编码为平台能力。
- 同次审计确认 GitHub 原生 `Merge pull request` 会在 base 上生成新的 merge commit，squash 会生成新的 squash commit，GitHub rebase 会始终改写为新 SHA；启用 linear history 又只允许 squash/rebase。因此在“不得 direct-main、不得 bypass、required workflow 必须通过 PR 执行”的约束下，当前 v1 的“base 落点等于已审核 PR head SHA”没有原生可执行路径。迁入已具备 GitHub Enterprise Cloud required-workflow 能力的 organization 只能解决 source-bound workflow/team 条件，不能单独解决 landing SHA；专用 attestor App 也不能单独解决 landing SHA。来源证明必须在“GitHub Enterprise Cloud source-bound required workflow”与“专用 attestor App”之间独立决策；落点必须另立并独立审核 landing-v2（例如显式验证落地 commit 的 parent/tree 与原始 PR-head 关系）或未来版本化 exact-landing 协议，二者不得捆绑成只有某一来源方案才能使用的组合。任何未来 exact-landing 协议若需要一次性 bootstrap bypass，必须显式冻结范围、lease、actor、reviewer 和失效条件，并在 non-merging probe 前撤销；probe 还必须重新证明 bypass 为空，当前 v1 不因此获得 bypass 例外。landing 协议、所需远端权限与第二 reviewer 未另获授权并完成独立审核前不得实施 `H0/A0/C0`；`C0` 精确落地后才创建 non-merging probe 并执行 active-block，全部通过前始终保持 `E0=false`。实施前必须重新复核 [GitHub merge methods](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/configuring-pull-request-merges/about-merge-methods-on-github)，不得凭本地 Git 语义猜测 GitHub 落点。
