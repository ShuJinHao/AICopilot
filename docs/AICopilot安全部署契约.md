...
- 仅 migration 尚未启动的可逆失败允许恢复并复验 support、compose、release state、PostgreSQL、RabbitMQ、Qdrant、HttpApi、DataWorker、RagWorker 和 Web；进程 Running、非 Restarting、非 OOM、RestartCount 稳定，且已有 Health 为 healthy 才能称恢复完成。持久化 started marker 后的任何失败都保持 blocked-partial、`resumeAllowed=false`，禁止脚本猜测回滚数据库或把旧 runtime拉回已迁移 schema。
