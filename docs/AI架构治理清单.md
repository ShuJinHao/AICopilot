...
| `AI-TEST-002` | P0 | Partial | 物理图已收口为 17 个 required runner、3 个 Manual runner、5 个 TestKit，required 为 1011；声明迁移为 old 785 / current 768 / retained 632 / replaced 151 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。旧串行绿色 run `29506936001` 为 33 分 25 秒；docs-only run `29510008812` 暴露共享 PowerShell runspace binder 竞态；并行化首红 run `29512024152` 继续暴露 governance 独立依赖和旧 DeploymentTests YAML 断言，均保全且未重跑。修复 HEAD `4d121ce576741a4fbf6bc0c14fcfcc398e06d515` 的 run `29513177407` 已首次完整全绿：总墙钟 23 分 02 秒，98/98 behavior、17 runner / 1011 case、0 failed / 0 skipped、coverage line `71454/87937`、branch `8399/15581`、deployment 33、Vitest 165、Playwright 43、mutation 100%、artifact fan-in 与最终 reconcile 全部通过；最终 artifact ID `8382189827`、digest `sha256:7f18bb57459a5a1f6e3f2bee1d1ff814a765be19443e788df6c413c6a84910e2`。唯一剩余远端提示是 download-artifact v4 的 Node 20 弃用 warning，当前已固定到官方 Node 24 的 v8 SHA 并同步行为/DeploymentTests 门禁。 | 等待 v8 pin HEAD 再次完整绿色且五 job annotations 全零；随后仅提交状态关单文档并让最终 docs-only HEAD 再跑一次，形成至少两个当前拓扑绿色时长样本。 |
...
| `AI-TEST-UI-001` | P1 | Partial | Vitest 28 文件/165 case 与 Playwright desktop/mobile 43 case 已进入唯一 required reconcile；run `29512024152` 的独立 Web/deployment lane 已通过 deployment 33、Vitest 165、Playwright 43，但其它 lane 首红使最终 fan-in 未执行。Run `29513177407` 已在完整绿色并行拓扑中再次通过 deployment 33/33、Vitest 165/165、Playwright 43/43、0 failed / 0 skipped / 0 flaky，并由 final `build-test` 成功下载、合并和对账，最终 artifact ID `8382189827`。Viewport 差异仍由 project-specific tag 物理发现，不使用运行后 Skip。 | 等待 v8 pin HEAD 与最终状态关单 docs-only HEAD 各完整通过 Web/deployment lane 和 final reconcile，再更新为 `Done`。 |
