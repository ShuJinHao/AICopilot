...
- 当前 AI P0 候选状态固定为 `AI-P0-LOCAL-CANDIDATE / REMOTE-PENDING`：源码静态审查和定向失败后移已经闭合，但最后一次 TestKit/FQN/baseline 改动后的 clean commit、Release build、重生 inventory、完整 required/coverage/mutation/Web/deployment/final reconcile 仍待执行，不得写成 Done 或 `AI-P0-LOCAL-CANDIDATE-COMPLETE`。
...
| `AI-TEST-002` | P0 | Partial | 历史治理证据保持不变：`59b93f...` 的 PR #60 run `29562903935` attempt 1 对旧图 required 1012/1012、Vitest 165、Playwright 43、deployment 33、coverage/mutation 和 final reconcile 全绿，`AI-W0` 合并基线为 `b40ee21...`。当前 P0-only case baseline 为 1122（required 1108 / Manual 14），活动 Vitest 基线为 168；4 个误导 FQN 已按当前权威 owner 改名，declaration transition 候选随之精确更新为 old 785 / current 845 / retained-self 624 / replaced 159 / retired-duplicate 0 / retired-source-guard 2 / unknown 0，NUL/LF content SHA-256 为 `4a079e7f0a8145125ac0b929a4a8d12b19c473dea73b08bb166c16d40783964f`，静态 transition validator 已通过。canonical attempt-01 使用 event-bound quality base `59b93f...` 后在 compatibility 门精确首红，证明旧 `29f927...` 无 baseline 的 bootstrap 口径不能授权新增 compatibility ID；当前修复删除该新 ID 与三个 legacy member，把 completed pre-v2 Plan 收口为 opaque raw `PlanJson` 只读历史，metadata 统一 `Invalid` / non-executable，candidate ledger 恢复为 base 既有 4 项。该修复后的 compatibility 精确统计和当前图 full 17、coverage、final reconcile 均尚未运行，因此保持 Partial，不改写旧 run 事实。 | 当前精确 HEAD 必须通过 event-bound quality base、完整 required+coverage、mutation、Web/deployment、同 run artifact fan-in 和 final reconcile；任何失败不得用 rerun、Skip、过滤、旧 1012/165 证据或跨 run artifact 覆盖。 |
...
| `P0-010` | P0 | Partial | 已建立唯一 `AgentPlanCanonicalizer`、Plan v2 Draft canonical bytes/digest、typed IntentCandidate/Node/Evidence/ExecutionSnapshot、严格 Tool input/output schema 与稳定 Cloud 写/PLC/`DagV1` 拒绝；P0 没有可信 compiler 时生产 `ExecutablePlan` 固定 fail-closed，不接入生产 TestKit harness 或影子 compiler。当前 Application 首红修正后，retired auto Skill selector 的 match/no-match 两例均精确证明 `CallCount=0`；unsupported input schema 的真实 owner 为 `ToolRegistration` 构造边界，confirmation 则在 missing compiler 处先 fail-closed。4 个 FQN 和 declaration transition 已同步这些真实 owner；定向测试曾通过，但最后 TestKit/FQN 改动后的 full gates 尚未执行，因此保持 Partial。 | 在当前候选 clean HEAD 完成完整 required/Contract/HttpIntegration/Vitest 与 final reconcile；持续禁止 Tool→Skill 猜测、旧字段消费、P1/P2 compiler 或内部拒绝 detail 外泄。 |
| `P0-020` | P0 | Partial | Canonical Plan 已接入 Draft 生成/确认、聚合完整保存、真实 PostgreSQL fresh-context 校验、runtime fresh authorization、DTO、稳定错误披露、SSE/前端 preview，以及 strict Tool input/output schema 与规范 durable output；历史 completed pre-v2 Plan 仅以 opaque raw `PlanJson` 原字节只读返回，不解析或投影为专用 legacy schema/status，metadata 统一 `Invalid` / non-executable，任何写回继续由 v2-only persistence policy fail-closed；生产 runtime 对 Draft/无可信 executable 同样 fail-closed。当前 P0 candidate 保持 Cloud tool 对外结果的安全 roster，仅返回 `status/resultType/sourceMode/isSimulation/rowCount/isTruncated/resultHash`，raw rows 留在内部 run state；需要验证 P2 downstream 行为的现有测试显式使用 TestKit harness。HTTP fixture 从唯一 canonical built-in Plan owner 生成并把每个 sealed Step 原样投影到 aggregate，artifact 绑定真实 generator Step。定向 Application/Workflow/HTTP 证据已绿，但 compatibility 首红修复后的当前代码态最终全门待跑，故保持 Partial。 | 在 P0-only clean HEAD 复跑 exact 262144 REST/SSE round-trip、safe output roster、canonical Plan/approval/artifact 正反例、full required 和 Cloud provider tests；ArtifactWorkspace 原子 staging/reconciliation 与 finalization hardening 继续按 P1 专项治理。 |
| `P0-030` | P0 | Partial | Wave 1 的 P0-only candidate 已把 required 真实首红按 `Application 11 -> Workflow 1 -> HTTP 4 -> 2 -> 1 -> 0` 后移并以定向/full runner 诊断闭合；独立 reviewer 已通过生产零改动、generic Plan Step projection、4 个 FQN/transition mapping 与 P1 deny 静态核对。当前源码声明仍为 1122 case（1108 required / 14 Manual、0 Skip），transition 为 current 845 / retained 624 / replaced 159，活动 Web 基线为 Vitest 168；HttpIntegration 新增 TestKit consumer edge 后旧 inventory/TRX/binary 已明确 stale，不能冒充当前代码态验证。当前状态仅为 `AI-P0-LOCAL-CANDIDATE / REMOTE-PENDING`，最终 diff 冻结和 clean 全门仍待执行。 | 形成暂定 P0 implementation commit，并在只含该 commit 的 clean validation worktree 完成 Release build、真实 inventory、17 required runner+coverage、mutation、Vitest 168、Playwright 43、Web build、deployment behavior 33 与 final reconcile；全部 0 failed / 0 skipped 后再更新状态。 |
