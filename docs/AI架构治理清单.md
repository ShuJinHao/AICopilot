...
| Rule ID | 状态 | 权威结论 | 自动化边界 |
| --- | --- | --- | --- |
| `AI-RULE-GOV-001` | Active | 当前有效约束必须存在于正式规则、专题契约、治理清单或部署指南；复盘和历史记录不得成为唯一规则来源。Rule ID 必须稳定唯一，复盘记录权威文件与标题锚点，不以行号定位，不改义或复用旧 ID。 | 当前无专用 Rule-ID/锚点 linter；只能运行既有文档/治理静态检查并人工核对，未接入 test/Analyzer/CI 前不得称为自动阻断。 |
| `AI-HISTORY-001` | Active | 历史复盘不是默认必读材料；仅在历史回归、冻结链路改动、实现与契约冲突、失败原因不明、同类问题曾发生或用户要求追溯时，按模块名、Rule ID、错误码或关键类型精准检索。命中唯一规则来源时先迁移再实现。 | 当前无专用自动门禁；`rg` 精准检索是执行程序，不得把全文阅读复盘恢复为默认动作。 |
...
| AI-SEC-049 | MEDIUM | Partial | 上传安全策略技术探测去重 | 权威边界已完整提取到 `docs/DDD聚合根边界.md` 的“上传安全策略与字节探测边界（AI-SEC-049）”。AiGateway 与 RAG 继续分别拥有 allowlist、限额、canonical MIME、scope 和拒绝语义；共享层只可报告 stream/header/MIME token/MZ/NUL/签名等技术事实。当前生产代码仍有重复实现，因此文档提取不构成关单 | 单一窄职责 primitive；seekable/non-seek、空/短流、MZ、NUL、空/参数化 MIME、支持签名、取消传播、原始流未 dispose、owned temporary 仅 dispose 一次的等价矩阵；两套 policy 行为不变，重复技术实现归零，Architecture/Backend 全量通过后才可 Done |
