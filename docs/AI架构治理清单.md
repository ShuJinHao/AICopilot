...
| `P0-020` | P0 | Partial | Canonical Plan 已接入 Draft 生成/确认、聚合完整保存、真实 PostgreSQL fresh-context 校验、runtime fresh authorization、DTO、稳定错误披露、SSE/前端 preview，以及 strict Tool input/output schema 与规范 durable output；历史 completed v1 仅只读，生产 runtime 对 Draft/无可信 executable fail-closed。当前 P0 candidate 保持 Cloud tool 对外结果的安全 roster，仅返回 `status/resultType/sourceMode/isSimulation/rowCount/isTruncated/resultHash`，raw rows 留在内部 run state；需要验证 P2 downstream 行为的现有测试显式使用 TestKit harness。HTTP fixture 从唯一 canonical built-in Plan owner 生成并把每个 sealed Step 原样投影到 aggregate，artifact 绑定真实 generator Step；production source 修改仍为 0。定向 Application/Workflow/HTTP 证据已绿，但当前代码态最终全门待跑，故保持 Partial。 | 在 P0-only clean HEAD 复跑 exact 262144 REST/SSE round-trip、safe output roster、canonical Plan/approval/artifact 正反例、full required 和 Cloud provider tests；ArtifactWorkspace 原子 staging/reconciliation 与 finalization hardening 继续按 P1 专项治理。 |
