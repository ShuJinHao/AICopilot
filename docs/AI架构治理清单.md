...
| `AI-ARCH-001` | P0 | Planned | 现有 Architecture runner 仍包含源码字符串/Regex，本波只补充 AI-SEC-051 与 Agent compensation 动态 owner，未创建空 Analyzer。 | 使用 `Microsoft.CodeAnalysis.CSharp 5.6.0` 建立真实 Roslyn/MSBuild Analyzer 与 AnalyzerTests；跨层、循环依赖、直接 DbContext/SQL、绕聚合根、插件越界和 Cloud-readonly 写路径以编译 error 阻断。 |
