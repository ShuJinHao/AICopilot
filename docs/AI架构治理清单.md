...
| `AI-TEST-002` | P0 | Partial | 物理图已收口为 17 个 required runner、3 个 Manual runner、5 个 TestKit，required 为 1011；声明迁移为 old 785 / current 768 / retained 632 / replaced 151 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。实现 HEAD `1925b493b57eb484230fe2624b944fe4412e57e7` 的 run `29506936001` 已完整通过 build 0 warning/0 error、98/98 behavior、17 runner / 1011 case、coverage line `71454/87937`、branch `8399/15581`、deployment 33、Vitest 165、Playwright 43、final reconcile 和 mutation 100%。但 docs-only HEAD `9de129a42f147e47edf65849b8c5900f9f0c58d9` 的 run `29510008812` 在任何 `dotnet test` 启动前暴露 `ForEach-Object -Parallel` 共享 PowerShell host 直接调用 binder 的非确定性集合竞态；mutation 仍为 100%，required artifact ID `8380676888`、digest `sha256:735e53a5169fa3baeaf5a9067c666d6b28563afac9cf853afb9c7b86edcaffd1`，首次失败现场保存在 `/private/tmp/aicopilot-final-docs-run-29510008812-first-failure`。当前修复让每个 runner 通过隔离 `pwsh -NoProfile -File` 绑定输入，并把治理、.NET、Web/deployment、mutation 拆为同 run 并行 lane，由唯一 `build-test` 对全部 conclusion 和本 run artifacts 做 fail-closed 汇总。 | 等待并行化修复 HEAD 完整通过全部 lane、1011 required、coverage、Web/deployment、mutation 与最终汇总；随后提交状态关单文档并让最终 docs-only HEAD 再跑一次，形成两个当前拓扑绿色时长样本。 |
...
| `AI-TEST-UI-001` | P1 | Partial | Vitest 28 文件/165 case 与 Playwright desktop/mobile 43 case 已进入唯一 required reconcile；本地和 PR #60 run `29506936001` 均真实通过 Vitest 165/165、Playwright 43/43、0 failed / 0 skipped / 0 flaky，随后 Web production build 与 final reconcile 成功，Playwright JSON SHA-256 为 `d94f1c57871cfc46e6756ac4896a4132ca277c1aa7c626d84c06afdcec88adb9`。Run `29510008812` 因 .NET runner 启动前的 binder runspace 竞态 fail-fast，Chromium、Vitest、Playwright 和 final reconcile 未执行，不能作为最终 HEAD 的 UI 证据。Viewport 差异仍由 project-specific tag 物理发现，不使用运行后 Skip。 | 等待新的并行 Web/deployment lane 与最终 `build-test` 汇总在修复 HEAD 和状态关单 docs-only HEAD 上各完整通过 Vitest 165、Playwright 43、零失败/零跳过/零 flaky。 |
