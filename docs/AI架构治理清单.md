...
| `AI-TEST-002` | P0 | Partial | 物理图保持 17 个 required runner、3 个 Manual runner、5 个 TestKit，required 已由真实发现增至 1012；声明迁移为 old 785 / current 769 / retained 633 / replaced 150 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。本批恢复 `RagMcpEndToEndTests.RagAndMcpSmoke_ShouldIndexDocument_SearchContent_AndLoadOnlyEnabledMcp` 的可执行身份，精确 FQN 已由 0-hit 变为恰好 1-hit并在真实 Aspire 中通过 1/1；同时把 `AI-COMPAT-SECRET-ENCV1` 的 `IsLegacyEncrypted` / `ReEncryptLegacyCipher` 符号并集按两个真实生产调用成员计数，最大值仍为 2。GitHub run `29555985167` 已证明新 HEAD 的 governance、mutation、Web/deployment 三 lane 成功，17 runner / 1012 required 与 EndToEnd 25/25 均为 0 failed / 0 skipped；coverage 仅因 Linux 实测进一步提升而按 bootstrap exact 规则首红，现据原始日志把 `minimumLineRate` / `minimumBranchRate` 向上收紧为 `0.81262722` / `0.53918234`，其它 coverage 身份不变。旧 run `29513177407` / `29516004891` 的 1011-case 证据仅保留为历史样本，不能证明当前 1012-case HEAD。 | 当前 PR #60 新 HEAD 必须重新完整通过 102/102 behavior、17 runner / 1012 required、0 failed / 0 skipped、coverage/mutation、deployment 33、Vitest 165、Playwright 43、同 run artifact fan-in 与 final reconcile；失败则继续保全首红，不得用旧 run 关单。 |
