...
| `AI-TEST-002` | P0 | Partial | 当前物理图已收口为 17 个 required runner、3 个 Manual runner 和 5 个 TestKit；Release evaluated inventory 为 required 1011 / Manual 14。785 条旧声明账本锚定真实 Git `src/tests` 冻结树和有序符号摘要，当前结果为 current 768 / retained 632 / replaced 151 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。最终统一 Release build 为 0 warning / 0 error；17 个 required runner 已在同一 build 上全量重跑并对账 1011/1011、0 failed、0 skipped，其中 Aspire EndToEnd 24/24、HttpIntegration 29/29、真实 PostgreSQL Persistence 64/64。统一 required summary 同时确认 Vitest 28 files / 165/165、Playwright 43/43、deployment behavior 33/33，全部 0 failed / 0 skipped，Playwright 0 flaky。Mutation 已收紧为 73 generated / 58 evaluated / 58 killed、score 100%、0 survived / 0 no-coverage；compatibility、duplication、declaration transition 和 98/98 infrastructure behavior 均 no-update 通过。当前 clean full-universe coverage 对账 17 份逻辑报告 / 34 份物理副本、28 个生产程序集、757 个生产源码，line `71454/87937 = 0.81255899`、branch `8399/15581 = 0.53905398`。Draft PR #60 run `29494658997` 已再次证明 mutation 100% 与 17 runner / 1011 case 全绿，但 coverage 在首个 Aggregate runner 精确拒绝 `AICopilot.Core.AiGateway` runner DLL/PDB 与 inventory-bound canonical 输出不同，随后 Web/deployment/final reconcile 按 fail-fast 未执行。当前修复把 checkout HEAD 作为整份 solution build 的唯一 `SourceRevisionId`，inventory 精确冻结每个 runner 的 evaluated runtime closure，并让并行/串行 runner 紧邻执行前分别生成 launch binding；无显式开关时字节不一致失败，显式同步也只能修复 closure 内 pair，closure 外、HEAD/inventory 漂移、TRX target 或 post-test 字节不一致均失败。 | 等待 PR #60 新 required run 完整通过 coverage、deployment 33、Vitest 165、Playwright 43、最终 reconcile 与 annotations/artifacts，再进入三仓联合验收。调查、首红保全和 runner staging 修复不单独计入完成度。 |
...
| `AI-TEST-UI-001` | P1 | Partial | Vitest 28 文件/165 case 与 Playwright desktop/mobile 43 case 已进入唯一 required reconcile；最终本地 Vitest 为 165/165、0 failed / 0 skipped，Playwright 在 Node 22 + 精确 Chromium rev1217 / `channel=chromium` 下完整执行 43/43、0 failed / 0 skipped / 0 flaky。首次 43/43 在 browser launch 前因全局 rev1217 仅 448 KiB、缺 Chrome Framework 而环境失败，原始 JSON 已独立封存；修复使用隔离临时浏览器目录，没有升级 Playwright、复用其它 revision、修改测试 filter 或覆盖首次失败。统一 required summary 已同时对账 .NET 1011、Vitest 165、Playwright 43 和 deployment 33 全部为零失败/零跳过。Viewport 差异由 project-specific tag 物理发现，不再用运行后 Skip；固定 Agent 槽位、会话水合/切换/恢复、审批、产物和错误语义均由真实前端行为覆盖。 | 本地产品断言已闭合；提交后仍需由新 stacked Draft PR 的远端 required lane 在新鲜 Node 22/Chromium 环境复验，远端 job 与 annotations 通过后再改为 `Done`。 |
