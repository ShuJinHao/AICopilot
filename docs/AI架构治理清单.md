...
| --- | --- | --- | --- | --- |
| `AI-TEST-GOV-MIG-001` | P0 | Bootstrap-Local-Green | 已在隔离候选中新增 base-owned wrapper、runtime validator、reference schema 和精确 92 条隔离 Git 对抗自测；覆盖 Immutable、AuthorizationOnly、Consume、Cancel、validator-only TrustUpgrade、精确 path/mode/SHA/count/manifest、baseline test-project roster、PR head SHA、candidate wrapper/self-test 绕过、replay、线性历史与完整 required workflow 闭包。TrustUpgrade 在 Describe 与 receipt 两个入口都对归一化前的 raw rule ID 数组强制真单例；Describe 五项必填参数和唯一支持的 anchor 方向是 base self-test 按 Ordinal 锁定的 literal contract，并在签发 receipt 前先验证 target workflow 静态闭包。target tree 中 canonical 以外的任一 workflow 均不得复用 `aicopilot-ci`、`migration-validator-selftest`、`build-test`、`required-final`、两个 trust marker 或 wrapper/validator/self-test/schema 精确路径；其顶层 `name`、`jobs`、job ID 和直接 job `name` 还必须使用严格 ASCII plain block 语法，quoted escape、block/flow、anchor/alias 或动态表达式一律 fail-closed，不能经 YAML 语义解码伪造同名 check。canonical target workflow 在 LF 规范化后必须整文件逐字节相等：`migration-validator-selftest` 与 `build-test` 无 `needs` 并行执行、分别 checkout 同一 PR head SHA，均以 step-scoped 只读 `github.token` HTTP header 获取 trusted `origin/main`，不持久化凭据，也不通过 artifact/cache/workspace 在两 job 间传递 candidate；前者只用 base-owned self-test/wrapper/schema 检验 candidate validator，后者只以 base wrapper/validator 作权威 receipt gate 并继续旧治理与全量测试。最小 `required-final` 不 checkout、不读取或执行 candidate，只以 `always()` 对两个 `needs.*.result == success` fail-closed 聚合；三 job、依赖、完整 suffix 和 EOF 均受精确闭包保护。release anchor 除扫描 `candidate..trustedBase` 外还显式检查未受信 candidate 端点：真实 root 或单父提交允许，merge 端点以 `HISTORY` 拒绝。v1 receipt 永久不得修改 wrapper/schema/self-test harness；当前未修改 workflow、旧 policy、baseline 或 waiver，未生成 receipt。 | 先独立审核并提交四个 bootstrap 资产；再从该可信 base 生成精确 pending receipt，并把授权提交与 workflow/policy 消费提交分离。消费候选必须由 base 提取 wrapper/validator，并由 base-owned self-test/wrapper/schema harness 对 candidate validator 运行精确 92 条 self-test；checkout 固定 PR head SHA。v1 仅支持无 merge 的线性历史；远端 ruleset 必须禁 merge commit、启用 strict up-to-date 并保留已检查 PR-head SHA，普通 synthetic merge 或改 SHA 的 merge queue 与 v1 不兼容。只有 base-owned canonical workflow 产生的 `aicopilot-ci/migration-validator-selftest`、`aicopilot-ci/build-test`、`aicopilot-ci/required-final` 三个 context 全部在 branch protection/ruleset 中设为 required，且三者绑定同一已审 PR-head SHA 与同一 base-owned provenance，同时存在独立 reviewer 后，才可称 trust root 激活；不得仅凭可由其他 workflow 伪造的同名 status context 作为激活证据。 |
| `AI-TEST-001` | P0 | Phase0-CI-Green | 唯一 policy/behavior/baseline/waiver、统一 failSkips runner、CODEOWNERS、Release 编译门禁和 25 分钟 `aicopilot-ci/build-test` 已建立；四个旧 xUnit 项目 freeze `All`，Backend 120 个测试源文件与 Vitest/deployment/support/runner 资产已内容冻结。前端会话权威投影批次已把 Vitest 扩展为 31 文件/184 条、Playwright 扩展为 46 runner case，并将治理负例扩展为 65 条；提交 `f345f94` 的 canonical run `29179011823` 在 12 分 04 秒通过，Simulation run `29179011826` 的两个 job 同时通过，三个 job annotations 均为 0。 | 首次 workflow migration 按 `AI-TEST-GOV-MIG-001` 激活后，必须把 base-owned canonical workflow 的 `migration-validator-selftest`、`build-test`、`required-final` 三个 status context 同时配置为 required，并锁定同一 PR-head SHA 与同一 base-owned provenance。仓库目前只有 PR 作者本人一个管理员协作者，required Code Owner review 会造成永久自锁；须先加入独立 reviewer，再启用该保护。此条件满足前不得称防自改 hard gate 完成。 |
...
canonical 第三次 run `29174501488` 验证部署恢复与证据失败传播已闭环；前端会话权威投影提交 `f345f94` 随后由 run `29179011823` 在 12 分 04 秒内重新完成 governance、Release build、repository/discovery、Architecture 91/91、legacy Eval 6/6、Backend 924/924、deployment 33/33、Vitest 184/184、Web build 与 required-result reconcile。Simulation run `29179011826` 的 `simulation-rc` 和 `backend-full-tests` 也通过，三个 check-run annotations 均为 0；该远端证据与 31 文件/184 Vitest、46 runner case 的当前冻结边界一致。
