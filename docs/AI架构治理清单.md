...
| --- | --- | --- | --- | --- |
| `AI-TEST-001` | P0 | Done | 已由项目元数据、分类表和真实声明生成 schema v3 case 级 inventory：25 个项目包含 20 个 runner（17 required / 3 Manual）和 5 个中性 TestKit support，当前 1026 case（required 1012 / Manual 14）。Runner/TestKit 引用只来自 Release MSBuild evaluated `ProjectReference` / `PackageReference` 图，覆盖隐式 `Directory.Build.*`、递归 import、生效复合条件、逐 TFM item 和 TestKit 传递闭包；评估失败或 identity 缺失 fail-closed。Required .NET 对账固定 `discovered>0`、`discovered=executed`、`failed=0`、`skipped=0`；Web/Shell/Deployment 额外精确对账 Vitest 165、Playwright 43、deployment behavior 33，任一数量不等即失败。102 个脚本行为用例（含 29 个 coverage omission guards）覆盖 InProcess 分类/运行时依赖合并、项目图/真实依赖、`AIARCH001`–`AIARCH007` 抑制面、完整 PDB coverage、报告身份、evaluated closure、runner launch mismatch/显式同步/closure-extra/HEAD 漂移、launch inventory digest、manifest 计数/digest/flag/pre-hash/membership、TRX case-preserving codeBase/storage/跨平台路径大小写、post-test 字节篡改与 closure-extra、兼容符号并集的 distinct caller-member 棘轮、缺数及各质量对账失败边界；canonical build 还必须把实际 checkout HEAD 固定为唯一 `SourceRevisionId`，每个 runner 紧邻执行前都要生成并持久化精确输入绑定。 | 持续保持 evaluated 项目图、项目元数据、case 基线、canonical runner 输入和实际执行结果一致；新增或迁移测试必须同步分类与对账。 |
| `AI-TEST-002` | P0 | Done | 物理图保持 17 个 required runner、3 个 Manual runner、5 个 TestKit，required 为 1012；声明迁移为 old 785 / current 769 / retained 633 / replaced 150 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。最终审计纠偏 HEAD `59b93f7145fbe2dc908aa35970566d25875b473c` 的 PR #60 run `29562903935` attempt 1 已完成五个 job 全绿：required 1012/1012、0 failed / 0 skipped，coverage line `71460/87937`、branch `8401/15581`，mutation 58/58，deployment 33、Vitest 165、Playwright 43 和同 run final reconcile 全部成功。`AI-W0` 又把该候选与 `main` 合并为 clean base `b40ee21b9bc248176e6d7e0c278e4c50101b1d59`，本地按同一 inventory 重跑四类最终对账仍全绿。 | 持续要求每个新 HEAD 通过 event-bound quality base、四个 producer lane、同 run artifact fan-in 和最终 reconcile；任何失败不得用 rerun、Skip、过滤或旧 artifact 覆盖。 |
...
| `AI-TEST-UI-001` | P1 | Done | Vitest 28 文件/165 case 与 Playwright desktop/mobile 43 case 已进入唯一 required reconcile。最终治理 HEAD `59b93f...` 的 run `29562903935` attempt 1 在独立 Web/deployment lane 真实通过 deployment 33/33、Vitest 165/165、Playwright 43/43、0 failed / 0 skipped / 0 flaky，并由 final `build-test` 成功下载、合并和对账；`AI-W0` clean merge base `b40ee21...` 本地复验仍为相同精确数量。Viewport 差异由 project-specific tag 物理发现，不使用运行后 Skip。 | 持续保持 deployment 33、Vitest 165、Playwright 43、零失败/零跳过/零 flaky；每个 P0 新 HEAD 必须由自身 required CI 重新证明。 |
...
| `P0-010` | P0 | Pending | 冻结唯一 Plan Canonicalizer，以及 IntentCandidate、LinearV1 Plan v2、Node、Evidence、ExecutionSnapshot 的版本化契约；建立 `IntentResult -> IntentCandidate` 明确适配，稳定拒绝 Cloud 写、PLC 和 `DagV1`。 | 契约、canonical bytes/digest、稳定拒绝码与正反测试完成；不得实现 P1 runtime。 |
| `P0-020` | P0 | Pending | 将 P0 契约接入草案生成、确认、完整持久化、fresh-context runtime、DTO、REST、SSE 和前端；Plan 上限 262144 UTF-8 bytes，Node/Tool input 上限 8000 UTF-8 bytes，移除静默截断与 DynamicPlanner 生产 DI。 | 全链路只消费 canonical plan；历史 completed v1 仅只读，非终态无效 v1/缺 digest 拒绝；`plan_payload_too_large` 完整传播。 |
| `P0-030` | P0 | Pending | 执行 P0 完整 required CI、coverage、mutation、Web/deployment、兼容性/重复度/迁移检查和证据复盘。 | 所有当前精确数量、0 Skip、质量 ratchet、文档、分阶段提交、Draft PR 与当前头 CI 完整闭环。 |
