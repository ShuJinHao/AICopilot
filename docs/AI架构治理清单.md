...
| `AI-TEST-002` | P0 | Partial | 物理图已收口为 17 个 required runner、3 个 Manual runner、5 个 TestKit，required 为 1011；声明迁移为 old 785 / current 768 / retained 632 / replaced 151 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。实现 HEAD `1925b493b57eb484230fe2624b944fe4412e57e7` 的 run `29506936001` 已完整通过 1011 required、coverage line `71454/87937`、branch `8399/15581`、deployment 33、Vitest 165、Playwright 43、final reconcile 和 mutation 100%，但总墙钟 33 分 25 秒。Docs-only run `29510008812` 在任何测试启动前暴露共享 PowerShell runspace binder 集合竞态；首次失败保存在 `/private/tmp/aicopilot-final-docs-run-29510008812-first-failure`。并行化 HEAD `c4417da911aec41f7bb88ce387734d0bbeb775b9` 的 run `29512024152` 已证明隔离 `pwsh` binder 越过旧竞态、四 lane 并发、Web/deployment 与 mutation 独立全绿、`build-test` 能按 `always()` 正确汇总失败；该 run 的两个首红是 governance lane 未独立安装 TypeScript 依赖，以及 DeploymentTests 仍锁旧单 job YAML。当前修复已给 governance lane 增加 lockfile `npm ci`，并把 DeploymentTests 更新为精确断言四 lane、隔离 binder、同 run artifact fan-in 与 fail-closed 汇总。 | 等待修复 HEAD 完整通过全部 lane、1011 required、coverage、Web/deployment、mutation、artifact fan-in 与最终汇总；随后提交状态关单文档并让最终 docs-only HEAD 再跑一次，形成两个当前拓扑绿色时长样本。 |
...
| `AI-TEST-UI-001` | P1 | Partial | Vitest 28 文件/165 case 与 Playwright desktop/mobile 43 case 已进入唯一 required reconcile；本地和 PR #60 run `29506936001` 均真实通过 Vitest 165/165、Playwright 43/43、0 failed / 0 skipped / 0 flaky。新并行拓扑的 run `29512024152` 中独立 Web/deployment lane 用时 5 分 17 秒并再次通过 deployment 33、Vitest 165/165、Playwright 43/43、0 failed / 0 skipped / 0 flaky，artifact ID `8381178488`、digest `sha256:5c82851686d3a97b2e788e681647de7f30619cc1d58b9f7770f155706026cb9a`；但其它 lane 失败使最终 artifact fan-in/reconcile 未执行，不能作为最终 HEAD 关单。Viewport 差异仍由 project-specific tag 物理发现，不使用运行后 Skip。 | 等待并行 Web/deployment lane 与最终 `build-test` 汇总在修复 HEAD 和状态关单 docs-only HEAD 上各完整通过 Vitest 165、Playwright 43、零失败/零跳过/零 flaky。 |
