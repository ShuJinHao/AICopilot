...
| --- | --- | --- | --- | --- |
| `AI-TEST-001` | P0 | Done | 已由项目元数据、分类表和真实声明生成 case 级 inventory：24 个项目包含 19 个 runner（16 required / 3 Manual）和 5 个中性 TestKit support，当前 982 case（required 968 / Manual 14）。Runner/TestKit 引用只来自 Release MSBuild evaluated `ProjectReference` / `PackageReference` 图，覆盖隐式 `Directory.Build.*`、递归 import、生效复合条件、逐 TFM item 和 TestKit 传递闭包；评估失败或 identity 缺失 fail-closed。Required .NET 对账固定 `discovered>0`、`discovered=executed`、`failed=0`、`skipped=0`；Web/Shell/Deployment 额外精确对账 Vitest 185、Playwright 43、deployment behavior 33，任一数量不等即失败。67 个脚本行为用例（含 15 个 coverage omission guards）追加覆盖 `AIARCH001`–`AIARCH007` 的 pragma、`SuppressMessage`、`NoWarn`、`.editorconfig` 和 `.globalconfig` 抑制负例，并保持既有项目图、完整 PDB coverage、报告身份、缺数及各质量对账失败边界。 | 持续保持 evaluated 项目图、项目元数据、case 基线和实际执行结果一致；新增或迁移测试必须同步分类与对账。 |
| `AI-TEST-002` | P0 | Partial | 当前物理图已收口为 16 个 required runner、3 个 Manual runner 和 5 个 TestKit；Release evaluated inventory 为 required 968 / Manual 14。785 条旧声明账本锚定真实 Git `src/tests` 冻结树和有序符号摘要，当前结果为 current 735 / retained 635 / replaced 148 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。DataWorker 真实 `Program` 与 ArchitectureTests 现共用同一生产组合入口，精确验证 worker identity、3 个 hosted service、maintenance options 绑定与无效边界，未使用空反射换 coverage。Analyzer suppression 与动态 MCP 信任阻断修正将 required 从 954 依次收紧为 960/966/967/968；旧批次不得作当前证据。最终本地 clean committed HEAD 证据在 `artifacts/test-results-ai-closeout-final/` 对账 .NET `968/968`、Vitest `185/185`、Playwright `43/43`、deployment `33/33`、Simulation `12/1`，全部 0 failed / 0 skipped；Analyzer DLL/PDB 在 inventory 与 runner 后哈希不变。Coverage 以正式 update 重生后再 clean no-update 通过，16 份逻辑报告观察 34 个生产程序集、762 个生产源文件，line `0.8074643`、branch `0.50616506`，已删 `McpTargetTrustPolicy.cs` ghost 已由正式基线重生移除。 | 提交 PR 后，由远端 required lane 使用新鲜 Chromium 环境重新对账同一精确基线；远端 .NET 968、Vitest 185、Playwright 43 和 deployment 33 全部 0 failed / 0 skipped 后才可改为 `Done`。调查和清理不单独计入完成度。 |
