...
| `P0-020` | P0 | Partial | Canonical Plan 已接入 Draft 生成/确认、聚合完整保存、真实 PostgreSQL fresh-context 校验、runtime fresh authorization、DTO、稳定错误披露、SSE/前端 preview，以及 strict Tool input/output schema 与规范 durable output；历史 completed v1 仅只读，生产 runtime 对 Draft/无可信 executable fail-closed。当前 P0 candidate 新增 shared canonical bounded measure：排序/转义/数字/root exclusion 与结构限制共用 owner，exact `262144` 成功、`+1` 和远超固定短路为 `max+1`，Seal 用 64-hex placeholder 保持真实 SHA byte count 并将越界映射为 `plan_payload_too_large`，未放宽全局 preflight。Cloud typed plan 继续由唯一 `CloudAiReadRowLimitPolicy` 固定 `1..100`，并收紧 `Analysis.Process.Detail`：keyword-only 在 provider/HTTP 前拒绝，code/name 必须有 nonempty exact filters，processId 直查必须唯一且身份一致。runtime snapshot、ArtifactWorkspace aggregate/file identity binding、终审 provenance/状态校验与 staging/reconciliation 均保留在 P1 residual，不计入本 P0。 | 在 P0-only clean HEAD 复跑 exact 262144 REST/SSE round-trip 与 repository-not-called 超限反例、full required 和 Cloud provider tests；ArtifactWorkspace 原子 staging/reconciliation 与 finalization hardening 继续按 P1 专项治理。 |
| `P0-030` | P0 | Partial | Wave 1 已形成 P0-only index candidate：候选源码静态重算 1122 case（1108 required / 14 Manual、0 Skip）、transition current 845 / retained 628 / replaced 155，活动 Web 基线为 Vitest 168；P1 finalization/WAF/TestKit consumer edge 未进入候选。此前 dirty-tree targeted 结果只能作为定位证据，当前 writer 未运行 build，也未用 stale Release binary 冒充候选验证；cached diff、candidate parser 与首次 clean Release build 双门仍须独立复核。 | 独立静态审计通过后形成暂定 P0 implementation commit，并在只含该 commit 的 clean validation worktree 完成首次 Release build、真实 inventory、17 required runner+coverage、mutation、Vitest 168、Playwright 43、Web build、deployment behavior 33 与 final reconcile；全部 0 failed / 0 skipped 后再更新状态。 |
