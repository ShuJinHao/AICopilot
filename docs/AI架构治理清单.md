...
| --- | --- | --- | --- | --- |
| `AI-TEST-001` | P0 | Phase0-Local | 已新增唯一 policy/behavior/baseline/waiver、统一 failSkips runner、CODEOWNERS、Release 编译门禁和 25 分钟 `aicopilot-ci/build-test`；四个旧 xUnit 项目 freeze `All`，Backend 120 个测试源文件与 Vitest/deployment/support/runner 资产已内容冻结。AI-SEC-051 补强后基线为 785 声明、976 静态投影、1022 Release runner case，并锚定独立审核、提交和远端绿测后的 `88a9687`。 | 独立提交并推送本批，验证新 `aicopilot-ci/build-test` 的 annotations/25 分钟墙钟并启用 required status context。仓库目前只有 PR 作者本人一个管理员协作者，required Code Owner review 会造成永久自锁；须先加入独立 reviewer，再启用该保护。此条件满足前不得称防自改 hard gate 完成。 |
...
Phase 0 证据边界：本地 policy 已用 61 个负向变异完成独立复核，包括 dot-directory、Test SDK、MSBuild 大小写/嵌套 props/targets、solution/workflow/SDK 漂移、support host、Vitest/deployment 缩减、package/runner 替换和测试体空心化。AI-SEC-051 补强后的第一次完整 Backend 为 898/924、0 Skip，26 个失败统一来自本机缓存的 amd64 RabbitMQ 在 Apple Silicon 上发生 112 秒 AMQP 握手超时，migration/seed 已完成且业务测试体未进入；拉取原生 arm64 同 tag、确认无残留 DCP/容器后，同一 Backend 命令为 924/924、0 Skip、4 分 56 秒，required-result 对账通过。安全批次已在 `88a9687` 提交并由 GitHub run `29170924668` 证明 Architecture 91/91、Backend 924/924、两个 job 成功且 annotations 均为 0；旧 Windows Simulation job 仍按 OSType 跳过 Linux Docker/PostgreSQL suite，仅执行 12/12 的 skip-profile acceptance，且 npm audit 暴露 1 个 ECharts moderate vulnerability，这两项分别保留为 `AI-TEST-003` 与 `AI-SEC-053`。新 `aicopilot-ci` 的 hosted-runner 25 分钟墙钟和 branch protection 仍未验证；仓库当前只有 `ShuJinHao` 一个管理员协作者，required Code Owner review 会让作者无法批准自己的 PR，因此必须先增加独立 reviewer。此平台前置满足前，同 PR 自改 policy/workflow 的风险未闭环。
