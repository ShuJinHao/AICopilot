...
| 编号 | 严重级 | 状态 | 范围 | 修复要求 | 验收 |
| --- | --- | --- | --- | --- | --- |
| AI-SEC-046 | HIGH | Partial | DbContext / Outbox / retry | `AI-PERSIST-01a` 已删除无生产者扫描与重复 Outbox ownership；`AI-PERSIST-01b` 已把普通 repository、独立 audit writer、AiGateway `Session` events 和 RAG delayed factories 收口到唯一 `PersistenceCommitEngine` / `RepositoryPersistenceCommitter`，使用官方 execution strategy、同事务 durable marker、fresh verification、显式 RAG Document sequence id 和稳定 503 commit-unknown 契约。仍未完成 `AI-PERSIST-01c` 的 Identity 事务、commit-unknown 文件对账/清理与 marker 生命周期，因此禁止合并或部署 | 01a：EF metadata/migration/全量；01b：真实 PostgreSQL pre-commit retry、DB identity replay、commit-ACK lost、fresh verifier transient/persistent failure、caller cancellation、业务+audit+outbox+marker 原子性 + migration ownership + 503 行为；01c：Identity/对账/retention 全量后关单 |

`AI-PERSIST-01b` 固定口径：业务代码不写 retry loop；每个 attempt 对业务 Context 只调用一次 `SaveChangesAsync(false)`，Outbox/audit/marker 使用同一 PostgreSQL transaction，commit 确认后才 accept/clear。COMMIT ACK 丢失由 fresh context 验证 marker；marker 写入后 caller cancellation 不终止 commit/verification；无法确认时返回 `persistence_commit_outcome_unknown` 且不自动重放、不删除 RAG 文件。`AuditTransactionCoordinator`、`RagIntegrationEventStager`、AiGateway/RAG Outbox mapping 和重复 PostgreSQL scratch fixture 已物理删除。`AI-SEC-046` 仍保持 `Partial`，01b 不能独立合并/部署，必须等待 01c 完成 Identity、文件对账/清理与 marker retention 后共同验收。
