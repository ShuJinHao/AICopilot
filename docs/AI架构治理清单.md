...
| `AI-TEST-002` | P0 | Partial | 当前物理图已收口为 16 个 required runner、3 个 Manual runner 和 5 个 TestKit；Release evaluated inventory 为 required 968 / Manual 14。785 条旧声明账本锚定真实 Git `src/tests` 冻结树和有序符号摘要，当前结果为 635 retained / 148 replaced / 2 retired-source-guard / 0 unknown。DataWorker 真实 `Program` 与 ArchitectureTests 现共用同一生产组合入口，精确验证 worker identity、3 个 hosted service、maintenance options 绑定与无效边界，未使用空反射换 coverage。本次 Analyzer suppression 与动态 MCP 信任阻断修正先使 required 基线从 954 上升为 960，Create/Update 缺失元数据和 NonCloud JSON 反例将它收紧为 966，聚合公开构造/更新 API 无安全默认值契约再收紧为 967，动态 MCP 缺失 canonical ToolName 的真实 fail-closed 反例最终收紧为 968；旧 954/960/966/967 批次及仍引用已删 `McpTargetTrustPolicy.cs` 的 coverage baseline 不得作当前关单证据，必须由 clean committed HEAD 的同批 .NET/Web/deployment/coverage/Simulation 重生后再记录精确结果。 | 提交 PR 后，由远端 required lane 使用新鲜 Chromium 环境重新对账同一精确基线；远端 .NET 968、Vitest 185、Playwright 43 和 deployment 33 全部 0 failed / 0 skipped 后才可改为 `Done`。调查和清理不单独计入完成度。 |
