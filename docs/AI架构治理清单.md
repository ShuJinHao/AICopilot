...
| --- | --- | --- | --- | --- |
| `AI-TEST-001` | P0 | Done | 已由项目元数据、分类表和真实声明生成 schema v3 case 级 inventory：25 个项目包含 20 个 runner（17 required / 3 Manual）和 5 个中性 TestKit support，当前 1025 case（required 1011 / Manual 14）。Runner/TestKit 引用只来自 Release MSBuild evaluated `ProjectReference` / `PackageReference` 图，覆盖隐式 `Directory.Build.*`、递归 import、生效复合条件、逐 TFM item 和 TestKit 传递闭包；评估失败或 identity 缺失 fail-closed。Required .NET 对账固定 `discovered>0`、`discovered=executed`、`failed=0`、`skipped=0`；Web/Shell/Deployment 额外精确对账 Vitest 165、Playwright 43、deployment behavior 33，任一数量不等即失败。98 个脚本行为用例（含 29 个 coverage omission guards）覆盖 InProcess 分类/运行时依赖合并、项目图/真实依赖、`AIARCH001`–`AIARCH007` 抑制面、完整 PDB coverage、报告身份、evaluated closure、runner launch mismatch/显式同步/closure-extra/HEAD 漂移、launch inventory digest、manifest 计数/digest/flag/pre-hash/membership、TRX case-preserving codeBase/storage/跨平台路径大小写、post-test 字节篡改与 closure-extra、缺数及各质量对账失败边界；canonical build 还必须把实际 checkout HEAD 固定为唯一 `SourceRevisionId`，每个 runner 紧邻执行前都要生成并持久化精确输入绑定。 | 持续保持 evaluated 项目图、项目元数据、case 基线、canonical runner 输入和实际执行结果一致；新增或迁移测试必须同步分类与对账。 |
| `AI-TEST-002` | P0 | Done | 当前物理图已收口为 17 个 required runner、3 个 Manual runner 和 5 个 TestKit；Release evaluated inventory 为 required 1011 / Manual 14。785 条旧声明账本锚定真实 Git `src/tests` 冻结树和有序符号摘要，当前结果为 current 768 / retained 632 / replaced 151 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。最终统一 Release build 为 0 warning / 0 error；17 个 required runner 已在同一 build 上全量重跑并对账 1011/1011、0 failed、0 skipped，其中 Aspire EndToEnd 24/24、HttpIntegration 29/29、真实 PostgreSQL Persistence 64/64。统一 required summary 同时确认 Vitest 28 files / 165/165、Playwright 43/43、deployment behavior 33/33，全部 0 failed / 0 skipped，Playwright 0 flaky。Mutation 已收紧为 73 generated / 58 evaluated / 58 killed、score 100%、0 survived / 0 no-coverage；compatibility、duplication、declaration transition 和 98/98 infrastructure behavior 均 no-update 通过。当前 clean full-universe coverage 对账 17 份逻辑报告 / 34 份物理副本、28 个生产程序集、757 个生产源码，line `71454/87937 = 0.81255899`、branch `8399/15581 = 0.53905398`。Draft PR #60 run `29494658997` 已再次证明 mutation 100% 与 17 runner / 1011 case 全绿，但 coverage 在首个 Aggregate runner 精确拒绝 `AICopilot.Core.AiGateway` runner DLL/PDB 与 inventory-bound canonical 输出不同，随后 Web/deployment/final reconcile 按 fail-fast 未执行。修复提交 `1925b493b57eb484230fe2624b944fe4412e57e7` 把 checkout HEAD 固定为唯一 `SourceRevisionId`，inventory 精确冻结 evaluated runtime closure，并为每个 runner 紧邻执行前生成 launch binding；PR #60 run `29506936001` 已完整通过 build 0 warning/0 error、98/98 behavior、17 runner / 1011 case、coverage、deployment 33、Vitest 165、Playwright 43、final reconcile 和 mutation 100%，两个 job annotations 均为 0。required artifact ID `8379950156`、digest `sha256:86588716d58929ef795d421f918c2595b103f8a2a33b72750cc9da66c4365839`；mutation artifact ID `8379140435`、digest `sha256:09a6321e13f14c5cc82c8f4a97be56bb68a282446330d0ea7a281f4dcbe0e7c3`。 | 持续保持 17 runner、1011 required、零失败/零跳过、精确 launch binding、full-universe coverage 与质量 ratchet；当前状态关单文档提交后必须由最终 HEAD 再跑一次 required CI。 |
...
| `AI-TEST-DUP-001` | P1 | Done | 已对生产 exact/near/structural clone、TestKit helper 和 test assertion flow 建立 schema v4 ratchet；实例身份为 `path+line`，同一文件的重复出现也逐实例计数。动态 MCP 修正中把 Create/Update 重复默认参数物理收口为两个真实命令及其 handler 共用的 mutation contract；新增 `AiToolConfiguredMcpMetadata` 作为聚合、runtime builder 和 policy typed overload 共用的规范安全输入；Analyzer 保留 Roslyn 可追踪的 7 个直接 descriptor 字段，只以 CompilationEnd Rule ID 集合收口 tag 判定；测试拒绝断言 helper 各有两个真实调用方。这些类型和 helper 均承担实际语义，不是无调用方 alias/wrapper。当前 no-update 基线为 productionExact `426 / 910 / 7280 / 17330`、productionNear `591 / 1531 / 12248 / 29029`、productionStructural `1139 / 3723 / 29784 / 95318`、testSupportHelpers `110 / 312 / 1872 / 4441`、testAssertionFlows `223 / 846 / 2538 / 3596`，不保留回涨余量。门禁同时锁定汇总与每个 signature；同文件增长负例已进入 98/98 infrastructure behavior。 | 后续只能收紧基线；更新 schema/baseline 前必须先有真实重复实现减少和指标下降，不得用汇总持平、signature swap/expand 或单纯重生基线换绿。 |
| `AI-TEST-UI-001` | P1 | Done | Vitest 28 文件/165 case 与 Playwright desktop/mobile 43 case 已进入唯一 required reconcile；最终本地 Vitest 为 165/165、0 failed / 0 skipped，Playwright 在 Node 22 + 精确 Chromium rev1217 / `channel=chromium` 下完整执行 43/43、0 failed / 0 skipped / 0 flaky。首次 43/43 在 browser launch 前因全局 rev1217 仅 448 KiB、缺 Chrome Framework 而环境失败，原始 JSON 已独立封存；修复使用隔离临时浏览器目录，没有升级 Playwright、复用其它 revision、修改测试 filter 或覆盖首次失败。PR #60 run `29506936001` 已在新鲜 Ubuntu/Node 22/Chromium 环境真实执行 Vitest 165/165 与 Playwright 43/43，全部 0 failed / 0 skipped / 0 flaky，随后 Web production build 与 final reconcile 成功，build-test annotations 为 0；Playwright JSON SHA-256 为 `d94f1c57871cfc46e6756ac4896a4132ca277c1aa7c626d84c06afdcec88adb9`。Viewport 差异由 project-specific tag 物理发现，不再用运行后 Skip；固定 Agent 槽位、会话水合/切换/恢复、审批、产物和错误语义均由真实前端行为覆盖。 | 持续保持 Vitest 165、Playwright 43、零失败/零跳过/零 flaky，并由最终状态关单 HEAD 的 required lane 再次复验。 |
