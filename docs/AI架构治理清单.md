...
| --- | --- | --- | --- | --- |
| `AI-TEST-001` | P0 | Partial | 测试治理机制和 `AI-W0` 历史基线仍由 schema v3 inventory、Release evaluated 项目图、102 个 behavior/29 个 coverage guard、canonical `SourceRevisionId` 与 per-runner binding 证明；`b40ee21...` 的固定历史数量仍是 1026 case（required 1012 / Manual 14）和 Vitest 165。当前 P0 dirty candidate 保持 20 runner（17 required / 3 Manual）+ 5 support，但真实 discovery 已 intentional 迁移为 1134 case（required 1120 / Manual 14、0 Skip），活动 Web 基线为 Vitest 168、Playwright 43、deployment behavior 33；HttpIntegration 新增 TestKit consumer edge 改变 evaluated 项目图，尚未在 clean committed HEAD 完成全门禁，因此暂退 Partial。 | 在本批 clean commit 上重生 `repositoryClean=true` inventory，完成全部 required TRX/coverage、当前 168/43/33 精确对账、mutation、Web build 与 final reconcile；继续要求 `discovered=executed`、0 failed / 0 skipped，禁止用 dirty discovery、过滤或旧 artifact 关单。 |
| `AI-TEST-002` | P0 | Partial | 历史治理证据保持不变：`59b93f...` 的 PR #60 run `29562903935` attempt 1 对旧图 required 1012/1012、Vitest 165、Playwright 43、deployment 33、coverage/mutation 和 final reconcile 全绿，`AI-W0` 合并基线为 `b40ee21...`。当前候选已 intentional 更新 case baseline 为 required 1120 / Manual 14、活动 Vitest 基线为 168；declaration transition 候选为 old 785 / current 857 / retained-self 629 / replaced 154 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。上述 dirty-candidate build/discovery/定向结果尚未由新 clean HEAD 的四个 producer lane、同 run artifact fan-in 和 final reconcile 证明，因此暂退 Partial，不改写旧 run 事实。 | 当前精确 HEAD 必须通过 event-bound quality base、完整 required+coverage、mutation、Web/deployment、同 run artifact fan-in 和 final reconcile；任何失败不得用 rerun、Skip、过滤、旧 1012/165 证据或跨 run artifact 覆盖。 |
...
| `AI-TEST-DUP-001` | P1 | Done | 已对生产 exact/near/structural clone、TestKit helper 和 test assertion flow 建立 schema v4 ratchet；实例身份为 `path+line`，同一文件的重复出现也逐实例计数。动态 MCP 修正中把 Create/Update 重复默认参数物理收口为两个真实命令及其 handler 共用的 mutation contract；新增 `AiToolConfiguredMcpMetadata` 作为聚合、runtime builder 和 policy typed overload 共用的规范安全输入；Analyzer 保留 Roslyn 可追踪的 7 个直接 descriptor 字段，只以 CompilationEnd Rule ID 集合收口 tag 判定；测试拒绝断言 helper 各有两个真实调用方。这些类型和 helper 均承担实际语义，不是无调用方 alias/wrapper。当前 no-update 基线为 productionExact `426 / 910 / 7280 / 17330`、productionNear `591 / 1531 / 12248 / 29029`、productionStructural `1139 / 3723 / 29784 / 95318`、testSupportHelpers `110 / 312 / 1872 / 4441`、testAssertionFlows `223 / 846 / 2538 / 3596`，不保留回涨余量。门禁同时锁定汇总与每个 signature；同文件增长负例已进入当前 102/102 infrastructure behavior。 | 后续只能收紧基线；更新 schema/baseline 前必须先有真实重复实现减少和指标下降，不得用汇总持平、signature swap/expand 或单纯重生基线换绿。 |
| `AI-TEST-UI-001` | P1 | Partial | 历史治理 HEAD `59b93f...` / `AI-W0` 对 28 文件、Vitest 165、Playwright 43、deployment 33 的绿色证据保持不变。当前 P0 candidate intentional 新增 `agentPlanV2Preview.spec.ts` 两项和 `chunkReducer.spec.ts` 一项，current-source full Vitest 已实测 29 files / 168 passed、0 failed / 0 pending / 0 skipped / 0 todo；活动 CI/reconcile 基线同步为 168，但 clean committed HEAD 的完整 Web/deployment lane、Playwright 43、Web build 与 final fan-in 尚未执行，因此暂退 Partial。 | 在当前 clean HEAD 精确通过 Vitest 168、Playwright 43、deployment 33、Web build、0 failed / 0 skipped / 0 flaky，并由 final reconcile 消费同 run evidence 后恢复 Done；Viewport 仍按项目 tag 物理发现。 |
...
| `P0-010` | P0 | Partial | 已建立唯一 `AgentPlanCanonicalizer`、Plan v2 Draft canonical bytes/digest、typed IntentCandidate/Node/Evidence/ExecutionSnapshot、严格 Tool input/output schema 与稳定 Cloud 写/PLC/`DagV1` 拒绝；P0 没有可信 compiler 时生产 `ExecutablePlan` 固定 fail-closed，不接入测试 harness 或影子 compiler。当前 dirty candidate 已把两个 selection mode 收紧为 exact string-only converter；真实 HttpIntegration 证明数字/未知 token 为 400 且不进入 session，legacy `skillCode/preferredToolCodes` 在 SSE handler 读取 session 前固定通用 schema error，Application 证明 command 在持久化前拒绝而无选择正例仍生成 PlanDraft。前端 wire 已删除两旧字段，旧选择仍存在时可见 fail-closed、0 HTTP。上述仅有定向证据，尚未由 clean HEAD 全门禁证明，因此保持 Partial。 | 在当前候选 clean HEAD 完成完整 required/Contract/HttpIntegration/Vitest 与 final reconcile；持续禁止 Tool→Skill 猜测、旧字段消费、P1/P2 compiler 或内部拒绝 detail 外泄。 |
| `P0-020` | P0 | Partial | Canonical Plan 已接入 Draft 生成/确认、聚合完整保存、真实 PostgreSQL fresh-context 校验、runtime 快照/授权、DTO、错误码、SSE/前端 preview、Tool durable output binding 和 FinalOutput checkpoint；历史 completed v1 仅只读，生产 runtime 对 Draft/无可信 executable fail-closed。当前 dirty candidate 新增 shared canonical bounded measure：排序/转义/数字/root exclusion 与结构限制共用 owner，exact `262144` 成功、`+1` 和远超固定短路为 `max+1`，Seal 用 64-hex placeholder 证明真实 SHA byte count 不变且 digest 加入造成的跨界返回 `plan_payload_too_large`，未放宽全局 preflight。Cloud typed plan 继续由唯一 `CloudAiReadRowLimitPolicy` 固定 `1..100`，并收紧 `Analysis.Process.Detail`：keyword-only 在 provider/HTTP 前拒绝，code/name 必须有 nonempty exact filters，processId 直查必须唯一且身份一致。上述只有 dirty-candidate 定向绿色；262144-byte Plan 的真实后端 HTTP/SSE AgentTask round-trip、clean full、`AI-PERSIST-01d / AI-SEC-047` 仍未闭环。 | 补齐 exact 262144 Plan 的真实 REST/SSE request round-trip 与 repository-not-called 超限反例；完成 clean-HEAD full required、Cloud provider tests、原字节 digest 与非生产 live。ArtifactWorkspace staging/reconciliation 继续按 P1 专项治理。 |
| `P0-030` | P0 | Partial | 当前 dirty candidate 已完成 current-source Release 全图 0 warning / 0 error、fresh Release inventory intentional migration（1134 case：1120 required / 14 Manual、0 Skip）、29 files / Vitest 168/168 和定向后端验证；本次 Cloud↔AI 对齐的最终串行 targeted 结果为 InProcess `CloudAiReadClientContractTests` 45/45、Application `SemanticDefinitionTests` 1/1、Simulation `CloudReadonlySimulationTests` 12/12，全部 0 failed / 0 skipped / 0 warning，独立静态审计 PASS。case/Vitest 活动 baseline 与 HttpIntegration -> TestKit consumer edge 均已改变，且尚未在 clean committed HEAD 上执行完整 required+coverage、mutation、Playwright/Web/deployment 和 final reconcile，因此不得称 P0 关单。 | 独立审计通过后形成本地 clean commit；在该精确 HEAD 完成 clean inventory、17 required runner+coverage、mutation、Vitest 168、Playwright 43、Web build、deployment behavior 33 与 final reconcile，全部 0 failed / 0 skipped 后再更新状态；Cloud live 与跨仓 digest 单独按跨契约顺序验收；不 push、不部署。 |

### 1.2.1 `P0-020` 测试守护矩阵

P0-020 修改生产接线前先冻结以下矩阵。表中的“当前缺口”是本批必须闭环的测试任务；不得用已退役的 `AICopilot.BackendTests`、字符串扫描或非 fresh context 自证。
...
| `IAgentDynamicPlanner` 生产 DI/解析/调用为零，不存在 alias/wrapper/shadow compiler | `PostFixClosureArchitectureTests` | 需由生产 DI 图和实际 coordinator 构造同时证明；P0 不得接入 `AgentPlanV2Compiler` | `AICopilot.ArchitectureTests` + `AICopilot.ApplicationTests` |
| Cloud 写、PLC、`DagV1` 在 Intent/契约/runtime 三层拒绝 | `AgentPlanContractV2Tests`、Golden/Workflow 安全测试 | 必须防止 Plan 持久化/runtime 新接线绕过已冻结拒绝 | `AICopilot.UnitTests` + `AICopilot.WorkflowTests` + `AICopilot.GoldenEvalTests` |
| `AI-PERSIST-01d / AI-SEC-047` 关单前 Evidence 仅 inline 65,536 UTF-8 bytes，`ArtifactReference` 稳定拒绝 | `AgentPlanContractV2Tests` | 需把 P1 硬门保留为自动化反例，不在 P0 实现 journal | `AICopilot.UnitTests` + `AICopilot.ArchitectureTests` |
