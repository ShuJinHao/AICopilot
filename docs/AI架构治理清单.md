...
| `P0-010` | P0 | Partial | 已建立唯一 `AgentPlanCanonicalizer`、Plan v2 Draft canonical bytes/digest、typed IntentCandidate/Node/Evidence/ExecutionSnapshot、严格 Tool input/output schema 与稳定 Cloud 写/PLC/`DagV1` 拒绝；P0 没有可信 compiler 时生产 `ExecutablePlan` 固定 fail-closed，不接入生产 TestKit harness 或影子 compiler。当前 Application 首红修正后，retired auto Skill selector 的 match/no-match 两例均精确证明 `CallCount=0`；unsupported input schema 的真实 owner 为 `ToolRegistration` 构造边界，confirmation 则在 missing compiler 处先 fail-closed。4 个 FQN 已同步真实 owner；定向测试曾通过，但最后 TestKit/FQN 改动后的 full gates 尚未执行，因此保持 Partial。 | 在当前候选 clean HEAD 完成完整 required/Contract/HttpIntegration/Vitest 与 final reconcile；持续禁止 Tool→Skill 猜测、旧字段消费、P1/P2 compiler 或内部拒绝 detail 外泄。 |
