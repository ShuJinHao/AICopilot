...
| `AI-TEST-002` | P0 | Done | 物理图已收口为 17 个 required runner、3 个 Manual runner、5 个 TestKit，required 为 1011；声明迁移为 old 785 / current 768 / retained 632 / replaced 151 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。共享 PowerShell runspace binder 竞态、governance 独立依赖缺失、旧 DeploymentTests YAML 断言和 download-artifact Node 20 warning 均已由保全首红后修复。当前四 lane + `always()` fan-in 拓扑已有两个完整绿色样本：run `29513177407` 总墙钟 23 分 02 秒，run `29516004891` 总墙钟 21 分 11 秒；nearest-rank p95 为 23 分 02 秒（n=2），低于 25 分钟目标，且未删测、Skip、过滤或放宽门禁。最终 v8 pin 样本完整通过 98/98 behavior、17 runner / 1011 case、0 failed / 0 skipped、coverage line `71454/87937`、branch `8399/15581`、deployment 33、Vitest 165、Playwright 43、mutation 100%、artifact fan-in 和 final reconcile，五个 job annotations 均为 0；最终 artifact ID `8383264428`、digest `sha256:10c37ae1f1671142b2b9ea3db488b0b1126585878b45d885fa7d8c5fbfd9476b`，mutation artifact ID `8382891230`、digest `sha256:6e86fda1f71dff030620650c301ab6a88f7d17dc61c24ea1157d2c834d3e5a11`。 | 持续保持 17 runner / 1011 required、零失败/零跳过、全量 coverage/mutation、同 run artifact fan-in 和两样本以上性能复核；本状态关单 docs-only HEAD 必须再完成一次最终 required CI，失败则退回 `Partial`。 |
...
| `AI-TEST-UI-001` | P1 | Done | Vitest 28 文件/165 case 与 Playwright desktop/mobile 43 case 已进入唯一 required reconcile。Run `29513177407` 与 Node 24 v8 pin run `29516004891` 均在独立 Web/deployment lane 真实通过 deployment 33/33、Vitest 165/165、Playwright 43/43、0 failed / 0 skipped / 0 flaky，并由 final `build-test` 成功下载、合并和对账；最终样本五个 job annotations 全零，统一 artifact ID `8383264428`。Viewport 差异仍由 project-specific tag 物理发现，不使用运行后 Skip。 | 持续保持 deployment 33、Vitest 165、Playwright 43、零失败/零跳过/零 flaky；本状态关单 docs-only HEAD 必须再完成一次最终 required CI，失败则退回 `Partial`。 |
