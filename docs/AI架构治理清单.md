...
| AI-SEC-051 | HIGH | Partial | 至少一个可用管理员 | `DisableUser` 的 last-admin count 在 ReadCommitted 并发下可被两个请求同时绕过，`UpdateUserRole` 也可直接降级最后一个 Admin；管理员生存不变量尚未跨 demote/disable/delete/seed 串行化。后续必须让所有会减少可用管理员数量的命令复用同一数据库级锁或等价受约束事务，并在锁内重读状态；禁止只靠每个 handler 的应用层 count | 真实 PostgreSQL barrier 覆盖两个 Admin 并发 disable、disable+demote、demote+delete 与 seed 竞争，任何交错最终至少一个 enabled Admin；单请求拒绝审计、失败 Result 回滚和成功差量角色更新保持通过 |
