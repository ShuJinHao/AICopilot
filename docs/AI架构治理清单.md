...
| `AI-TEST-002` | P0 | Done | 物理图保持 17 个 required runner、3 个 Manual runner、5 个 TestKit，required 为 1012；声明迁移为 old 785 / current 769 / retained 633 / replaced 150 / retired-duplicate 0 / retired-source-guard 2 / unknown 0。本批恢复 RAG/MCP E2E 可执行身份并把 encv1 两个公开 legacy surface 纳入同一 distinct-caller-member 棘轮，compatibility 上限仍为 2。首红 run `29555985167` 证明 1012/1012 执行成功并因 Linux coverage 提升按 exact-bootstrap 正确失败；收紧后的 HEAD `37d16c69a3ea5e94bdf76faa84a4dab2961914fc` 由 run `29557026686` 五个 job 完整证明：102/102 behavior、17 runner / 1012 required、EndToEnd 25/25、0 failed / 0 skipped、coverage line `71460/87937` / branch `8401/15581`、mutation 100%、deployment 33、Vitest 165、Playwright 43、同 run artifact fan-in 与 final reconcile 全部成功。统一 artifact ID `8398153853`、digest `sha256:e94d447a92dc1495763e276273230cf9d5a8619789ff5f7ac1b5ea388d7b4107`。 | 本状态关单会形成 docs-only HEAD；该最终 HEAD 必须再完整通过一次 required CI，失败则退回 `Partial`，成功后不得再修改 AICopilot 仓库或用后续 docs commit 使证据失配。 |
