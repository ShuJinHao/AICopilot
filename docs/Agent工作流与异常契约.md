...
- 严格审批元组必须唯一绑定 `taskId + workspaceId/workspaceCode + finalStepId + finalOutputApprovalId + activeRunAttemptId + task/node fencing token + EvidenceSetDigest + manifest digest + Artifact binding/file SHA-256`。封存审批时 task 与 attempt 的同一 run lease 必须均晚于审批创建时间；任一标识缺失、外来、重复、lease 已过期或在审批后漂移，都必须返回 `agent_finalization_state_conflict`，不能按相似 workspace、最近 attempt 或最后一个审批猜测。
- final-output `decision proof` 必须绑定 approval proof digest、审批创建时间、目标 workspace、唯一审批状态、决策人、决策时间和安全 comment；时间进入 digest 前按 PostgreSQL 可持久化的微秒精度规范化。`Pending` 不得携带决策字段，`Approved/Rejected` 必须有非空决策人与不早于创建时间的决策时间。审批请求必须晚于所有产物生成步骤完成；运行时必须在调用审批协调器前显式提交已完成前置步骤和 `WaitingApproval` 最终步骤，随后由审批存储在隔离事务中 fresh-read 并封存证明，禁止让未提交的 claim graph 参与裁决。只有 `Approved` 可以触发 durable resume，`Rejected` 立即终结任务并返回 `agent_approval_rejected`。
- 同一任务只能存在一个 final-output 审批。两个审批人竞争决定、批准与拒绝竞争、重复点击或迟到请求只能有一个决策胜出；相同已决结果可幂等读取，不同决策必须返回 `agent_approval_state_conflict`，不得覆盖首个 decision proof，也不得产生第二个关单队列项。已批准恢复在后续关单中进入 `Failed` 或 `Cancelled` 时，proof、source-bound 队列元组、attempt 与 fencing 仍完整的迟到相同批准仍须返回既有决定；下游终态通过 task/queue 独立呈现，不得把执行失败或取消倒灌为审批冲突，也不得重新 enqueue。`Approved` 与带唯一 `sourceApprovalRequestId` 的 `ApprovalResume` 必须由同一数据库事务提交，禁止审批保存后再 best-effort enqueue。
- Artifact 版本编辑必须在数据库提交期复核 final-review edit window，不能只依赖请求入口的预检。编辑提交与审批封存必须共享按 `task → workspace → artifacts` 稳定排序的 PostgreSQL 行锁；锁内 fresh-read 不再是 `WorkspaceReady`、workspace 已 Finalized 或已存在 FinalOutput 审批时，编辑事务与 staged file-set 必须一起回滚，禁止审批封存后再提交旧版本。
- 批准不是关单。审批请求完成后任务仍为 `WaitingFinalApproval`，attempt 先保持无 lease 的 `WaitingApproval`；durable Worker 必须复用同一 attempt 与 task fence 重新 claim，fresh-read proof、Evidence、源文件和数据库权威后才可执行 file-set stage 与 fenced checkpoint。文件哈希、复制或 stage 前后的 lease renewal 必须在同一数据库锁内要求 task、attempt、queue 与 NodeRun 四个当前 lease 均存在且严格晚于 renewal 时间；任一过期都返回 stale fence，禁止续活已失效 claim。只有 Artifact、Workspace、最终步骤、attempt、任务、最终 NodeRun/Evidence、file-set operation、`AgentTaskStepCompleted/ArtifactReady/FinalOutputReady` timeline 投影和结构化成功审计在同一权威提交中一致到达终态，任务才是 `Completed`；禁止权威提交后再 best-effort 补投影或审计。已有 Session 的所有 `messages/message_events` 分配器必须共享按 Session 稳定排序的数据库行锁，并在保存前依据两表共同最大值重排本次新增序号；同事务新建 Session 以 0 为基线分配首批序号。消息事件必须与其 Message 保持同一序号。
- checkpoint 损坏、manifest/file binding 不一致、lease 过期、fencing 失效、提交结果未知或恢复时间线断裂时必须 fail-closed；批准后发现的权威或源文件冲突必须保留 active attempt 与已批准末步，原子转为 manual `ReconciliationRequired` 并写审计，不得走普通失败关单。若冲突发生在 `ApprovalResume` 已 `Started`、最终 NodeRun 尚未 claim 的窗口，Worker 必须用当前 queue/task/attempt fence 将任务、attempt 和 `Agent.FinalizationReconciliationRequired` 审计同事务提交，并保留 `Started` 队列；source approval 行缺失、`taskId` 改绑或 `approvalType` 非 `FinalOutput` 时，仍以队列中的 `sourceApprovalRequestId` 作为审计目标完成对账，禁止因为损坏的审批权威无法再提供 ID 而返回普通状态冲突或循环恢复。重复分流幂等，过期恢复不得把对账队列恢复成普通 claim，审计提交失败必须整体回滚且不得随后 terminal-complete 为 `Failed`。无效 source-bound `ApprovalResume` 在 claim 前的 `DeadLetter`、任务/attempt 对账状态与同名审计也必须在同一事务尝试中提交，审计失败时全部回滚。不得从 Timeline、前端状态或目录是否存在反推完成。幂等恢复只能 fresh-read 权威 checkpoint/commit marker：已完成则返回既有结果，明确未提交才允许同一 fenced attempt 继续，结果未知进入 reconciliation，禁止重放审批、模型、Tool 或文件副作用。
- 只有异常链中明确存在 `IsTransient=true` 的 Npgsql provider 异常时，统一 persistence engine 才可重试整笔事务；辅助审计/Outbox context 包装的 transient 必须还原给外层策略，普通 `TimeoutException`、未知数据库异常和业务异常不得擅自升级为重试。每笔 Agent 执行事务必须从强类型 `DbContextOptions<AiGatewayDbContext>` 新建并独占 context，不得与同一 scope 中承载 task/workspace/step/attempt 的运行时 context 共享 ChangeTracker；`DbContextOptions<T>` 的完全限定 symbol 与程序集身份必须同时来自 `Microsoft.EntityFrameworkCore`，不得接受同名伪类型。每次失败只清空隔离事务 context，禁止脱离或替换外层运行时聚合。MigrationWorkApp 跨 AiGateway/RAG schema 修复密钥时，每个 execution-strategy attempt 必须分别新建两个 context、重新读取两张源表并共用一个数据库事务；secondary context 包装的 provider transient 可以还原给外层策略，未明确识别为 Npgsql transient 的包装异常不得重试。重试动作重新锁定并读取权威；运行结束后的队列裁决必须再次读取任务与 attempt，`ReconciliationRequired` 或权威记录缺失时不得调用 terminal completion。
- 旧同步入口 fail-closed：`POST /api/aigateway/workspace/{code}/finalize` 不得创建恢复队列、复制 final 文件、修改 Artifact/Workspace 或把任务标记完成；批准事务已经原子创建唯一 durable resume。该入口只允许对已 `Completed + Finalized` 的结果幂等读取，其余待审批、已批准待 Worker、拒绝、冲突、损坏或已被 lease 处理状态必须返回稳定结果/错误，不得降级调用旧 finalization service。
- 历史 proofless FinalOutput 只允许在任务和审批均为终态且每个任务最多一条时迁移为 `legacy-read-only-v0`；旧非终态、孤立或重复记录必须阻断迁移并要求显式修复，不得自动补 proof。以上是最终闭环的活动契约；源码、exact-SHA 验证与生产状态以 `AI架构路线图.md` 为准，源码闭环不等于生产验收。
