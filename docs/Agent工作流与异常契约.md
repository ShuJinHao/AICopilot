...
- final-output `decision proof` 必须绑定 approval proof digest、审批创建时间、目标 workspace、唯一审批状态、决策人、决策时间和安全 comment；时间进入 digest 前按 PostgreSQL 可持久化的微秒精度规范化。`Pending` 不得携带决策字段，`Approved/Rejected` 必须有非空决策人与不早于创建时间的决策时间。审批请求必须晚于所有产物生成步骤完成；只有 `Approved` 可以触发 durable resume，`Rejected` 立即终结任务并返回 `agent_approval_rejected`。
