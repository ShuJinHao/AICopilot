...
- checkpoint 损坏、manifest/file binding 不一致、lease 过期、fencing 失效、提交结果未知或恢复时间线断裂时必须 fail-closed；批准后发现的权威或源文件冲突必须保留 active attempt 与已批准末步，原子转为 manual `ReconciliationRequired` 并写审计，不得走普通失败关单。无效 source-bound `ApprovalResume` 的 `DeadLetter`、任务/attempt 对账状态与 `Agent.FinalizationReconciliationRequired` 审计必须在同一事务尝试中提交，审计失败时全部回滚。不得从 Timeline、前端状态或目录是否存在反推完成。幂等恢复只能 fresh-read 权威 checkpoint/commit marker：已完成则返回既有结果，明确未提交才允许同一 fenced attempt 继续，结果未知进入 reconciliation，禁止重放审批、模型、Tool 或文件副作用。
