...
- 只有异常链中明确存在 `IsTransient=true` 的 Npgsql provider 异常时，统一 persistence engine 才可重试整笔事务；辅助审计/Outbox context 包装的 transient 必须还原给外层策略，普通 `TimeoutException`、未知数据库异常和业务异常不得擅自升级为重试。每次失败必须清空该 Agent 执行 context 的 ChangeTracker，重试动作重新锁定并读取权威；运行结束后的队列裁决必须再次读取任务与 attempt，`ReconciliationRequired` 或权威记录缺失时不得调用 terminal completion。
