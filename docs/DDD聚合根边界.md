...
- 跨 AiGateway/RAG schema 的密钥迁移必须让每个 execution-strategy attempt 分别新建两个 context、重新读取两张源表并共享一个数据库事务；secondary context 包装的异常只有在异常链明确包含 `IsTransient=true` 的 Npgsql provider 异常时才可还原给外层策略，普通包装异常和业务异常不得触发重试。失败 attempt 必须清空其隔离 context 的 ChangeTracker，禁止脱离、替换或污染外层运行时聚合。
