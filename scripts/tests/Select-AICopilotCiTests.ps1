...
function Get-CiCategory {
    ...
    if ($Runtime -ceq 'LiveExternal') {
        return 'CrossProject'
    }
    switch ($TestKind) {
        'Architecture' { return 'Architecture' }
        'Deployment' { return 'DeploymentContract' }
        'GoldenEval' { return 'Quality' }
...
function Get-SolutionProjectPaths {
    ...
    try {
        ...
        throw "AICopilot solution XML is invalid: source=$Source"
    }
...
function Get-BaselineProjectDescriptor {
    ...
    try {
        ...
        throw "Baseline AICopilot project XML is invalid: ref=$Ref project=$ProjectPath"
    }
    ...
    $descriptor = [pscustomobject]@{
        ...
        ExplicitCategory = Get-DirectProjectProperty -Project $projectXml -Name 'AICopilotCiCategory'
        SecurityFilter = Get-DirectProjectProperty -Project $projectXml -Name 'AICopilotCiSecurityFilter'
        Category = ''
        References = @()
    }
...
function Add-BaselineBusinessImpact {
    ...
    if (-not $BaselineProject.IsTest -or
        ...
        -not [string]::IsNullOrWhiteSpace([string]$BaselineProject.SecurityFilter)) {
        return $false
    }
    $ownedSourceReferences = @($BaselineProject.References | Where-Object {
            -not $_.StartsWith('src/tests/', [StringComparison]::Ordinal) -and
...
if (-not (Test-Path (Join-Path $root 'AICopilot.slnx') -PathType Leaf)) {
    throw "AICopilot repository root is invalid: $root"
}
...
foreach ($project in $allProjects) {
    if ($project.IsTest) {
        ...
        if ($project.Category -notin @(
            ...
            throw "AICopilot test project has an invalid CI category: project=$($project.Path) category=$($project.Category)"
        }
    }
...
if ($Mode -eq 'Quality') {
    ...
    $selected.Clear()
    $liveProject = @($testProjects | Where-Object Category -ceq 'CrossProject')
    if ($liveProject.Count -ne 1) {
        throw 'CrossProject mode requires exactly one AICopilot CrossProject test project.'
    }
    Add-SelectedProject -Selected $selected -Project $liveProject[0] `
    ...
    foreach ($file in $changed) {
        ...
        if ($file -ceq $solutionPath) {
            ...
            foreach ($projectPath in $solutionDelta) {
                ...
                if ($null -eq $project -or -not $project.IsTest -or
                    $project.Category -cne 'Business' -or
                    -not [string]::IsNullOrWhiteSpace([string]$project.SecurityFilter)) {
                    $businessOnly = $false
                    break
                }
            }
        ...
        if ($file.StartsWith('deploy/', [StringComparison]::Ordinal) -or
            ...
            $deploymentAffected = $true
            $deploymentProject = @($testProjects | Where-Object Name -eq 'AICopilot.DeploymentTests')
        ...
        if ($baselineReady) {
            ...
            if ($baselineOwner.Count -eq 1) {
                ...
                if ($null -ne $baselineProject -and $baselineProject.IsTest -and
                    ...
                    [string]::IsNullOrWhiteSpace([string]$baselineProject.SecurityFilter)) {
...
if ($unclassified.Count -gt 0) {
    ...
    throw "AICopilot CI cannot safely attribute these files; no full-suite fallback was used. Run an explicit workflow_dispatch mode ($modes) after review:`n$(@($unclassified | Sort-Object -Unique) -join "`n")"
}

Write-Host "AICOPILOT_CI_SELECTION_OK mode=$Mode tests=$($selectedProjects.Count) web=$webAffected deployment=$deploymentAffected output=$resolvedOutput"
