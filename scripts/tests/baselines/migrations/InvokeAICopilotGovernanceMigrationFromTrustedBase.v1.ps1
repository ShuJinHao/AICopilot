...
$validatorRepositoryPath = 'scripts/tests/baselines/migrations/ValidateAICopilotGovernanceMigration.v1.ps1'
...
if ($RemainingArguments.Count -ne 0) {
    throw "AI-TEST-GOV-MIG-001-PARAMETER unexpected positional arguments: $($RemainingArguments -join ', ')."
}
foreach ($revision in @($TrustedBaseRevision, $CandidateRevision)) {
    if ($revision -notmatch '^[0-9A-Fa-f]{40}$' -or $revision -match '^0{40}$') {
        throw 'AI-TEST-GOV-MIG-001-REVISION trusted base and candidate must be explicit non-zero full SHAs.'
    }
}
...
function Invoke-GitText {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "AI-TEST-GOV-MIG-001-GIT git $($Arguments -join ' ') failed: $($output -join [Environment]::NewLine)"
    }
...
function Resolve-Commit {
    ...
    try {
        ...
        throw "AI-TEST-GOV-MIG-001-REVISION $Name is not an available commit: $Revision."
    }
    if ($resolved -cnotmatch '^[0-9a-f]{40}$') {
        throw "AI-TEST-GOV-MIG-001-REVISION $Name did not resolve to one full commit SHA."
    }
    return $resolved
}
...
function Export-GitBlob {
    ...
    if (-not $process.Start()) {
        throw 'AI-TEST-GOV-MIG-001-TRUST could not start git blob extraction.'
    }
    ...
    try {
        ...
        if ($process.ExitCode -ne 0) {
            throw "AI-TEST-GOV-MIG-001-TRUST git blob extraction failed: $errorText"
        }
    }
    finally {
        $process.Dispose()
...
if (-not $entryMatch.Success -or $entryMatch.Groups[2].Value -cne $validatorRepositoryPath) {
    throw "AI-TEST-GOV-MIG-001-TRUST trusted base does not contain the reviewed mode-100644 validator."
}
$validatorObjectId = $entryMatch.Groups[1].Value
$temporaryValidator = Join-Path ([IO.Path]::GetTempPath()) "$([Guid]::NewGuid().ToString('N')).aicopilot-governance-migration.ps1"
try {
    ...
    if ($extractedObjectId -cne $validatorObjectId) {
        throw 'AI-TEST-GOV-MIG-001-TRUST extracted validator differs from the trusted Git blob.'
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "AI-TEST-GOV-MIG-001-TRUST trusted validator failed with exit code $LASTEXITCODE."
    }
}
finally {
