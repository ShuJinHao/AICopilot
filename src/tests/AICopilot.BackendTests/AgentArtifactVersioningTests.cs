...
[Trait("Suite", "Batch8ArtifactVersioning")]
...
public sealed class AgentArtifactVersioningTests
{
    ...
    public async Task Owner_ShouldEditDiffDownloadAndRestoreDraftTextArtifact()
    {
        ...
        var owner = await CreateUserAsync($"batch8-owner-{Guid.NewGuid():N}", "User");
    ...
    {
        await AuthenticateAsAdminAsync();
        var owner = await CreateUserAsync($"batch8-lock-owner-{Guid.NewGuid():N}", "User");
        var seeded = await SeedWorkspaceReadyArtifactAsync(
            Guid.Parse(owner.UserId),
            ArtifactType.Markdown,
            "draft/report.md",
    ...
    {
        await AuthenticateAsAdminAsync();
        var owner = await CreateUserAsync($"batch8-cross-owner-{Guid.NewGuid():N}", "User");
        var other = await CreateUserAsync($"batch8-cross-other-{Guid.NewGuid():N}", "User");
        var seeded = await SeedWorkspaceReadyArtifactAsync(
            Guid.Parse(owner.UserId),
            ArtifactType.Markdown,
            "draft/report.md",
    ...
    public async Task UserWithoutEditArtifactPermission_ShouldReceiveMissingPermission()
    {
        ...
        var role = await CreateRoleAsync(
            $"Batch8ReadOnly-{Guid.NewGuid():N}",
            [
                "AiGateway.GetWorkspace",
                "AiGateway.DownloadArtifact",
                "AiGateway.SubmitFinalReview"
            ]);
        var owner = await CreateUserAsync($"batch8-readonly-{Guid.NewGuid():N}", role.RoleName);
        var seeded = await SeedWorkspaceReadyArtifactAsync(
            Guid.Parse(owner.UserId),
            ArtifactType.Markdown,
            "draft/report.md",
    ...
    {
        await AuthenticateAsAdminAsync();
        var owner = await CreateUserAsync($"batch8-pdf-owner-{Guid.NewGuid():N}", "User");
        var seeded = await SeedWorkspaceReadyArtifactAsync(
            Guid.Parse(owner.UserId),
            ArtifactType.Pdf,
            "draft/report.pdf",
        ...
        string mimeType)
    {
        ...
        var workspaceCode = $"ws_batch8_{Guid.NewGuid():N}"[..38];
        ...
        var step = task.AddStep(
            ...
            "Generate draft artifact for Batch 8 tests.",
