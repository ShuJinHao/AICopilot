...
public sealed class CloudAiReadClientTests
{
    ...
    [InlineData("PATCH", "/api/v1/ai/read/devices")]
    [InlineData("DELETE", "/api/v1/ai/read/device-logs")]
    public void EndpointPolicy_ShouldRejectEveryNonGetMethod(string method, string path)
    {
        var decision = CloudAiReadEndpointPolicy.Evaluate(new HttpMethod(method), path);
    ...
    [InlineData("{\"records\":[]}")]
    [InlineData("{\"results\":[]}")]
    [InlineData("{\"items\":[],\"asOfUtc\":\"not-a-date\",\"source\":\"devices\",\"queryScope\":\"\",\"rowCount\":0,\"truncated\":false}")]
    [InlineData("{\"items\":[],\"asOfUtc\":\"2026-07-10T01:02:03Z\",\"source\":\"devices\",\"queryScope\":\"\",\"rowCount\":-1,\"truncated\":false}")]
    [InlineData("{\"items\":[],\"asOfUtc\":\"2026-07-10T01:02:03Z\",\"source\":\"devices\",\"queryScope\":\"\",\"rowCount\":0,\"isTruncated\":false}")]
    public async Task Client_ShouldRejectLegacyOrMalformedProviderEnvelope(string payload)
    {
        using var httpClient = new HttpClient(new StubHandler(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
