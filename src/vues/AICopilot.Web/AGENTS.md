...
- 初始化期间允许用户编辑草稿、切换 Chat/Plan、展开高级选项并选择 Skill/Tool/知识库，但不得提交或上传。首次 `null -> A`（包含无缓存、有效缓存和 stale fallback）与 `A -> A` 刷新属于水合，必须保留这些 composer-local 状态；重新进入 ChatView 必须在子组件渲染前同步撤销旧 resolved-session 动作权限并清空旧 runtime 投影。
...
- `uploadedFiles` 与 composer 选择属于会话本地输入：`A -> A` 水合必须保留，真实 `A -> B` / 新建会话必须清空。初始化错误必须按来源保存，只能由同一来源成功重试清除；普通模式切换或清当前运行错误不得抹掉 Skill/Tool/知识库/session 水合失败。
