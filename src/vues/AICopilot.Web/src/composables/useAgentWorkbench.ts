...
export function resolveAgentRunNotice(task?: AgentTask | null): AgentRunNotice | null {
  ...
  if (task.status === 'Failed' && (task.failureSummary || task.lastFailureReason)) {
    return {
      ...
      detail:
        task.failureSummary?.safeMessage ||
...
export function useAgentWorkbench() {
  ...
  const draftArtifacts = computed(() =>
    store.currentWorkspace?.draftArtifacts?.length
      ...
      : taskArtifacts.value.filter(
          (artifact) => (artifact.artifactStatus || artifact.status) !== 'Final',
        ),
  )
  ...
  const draftArtifactCount = computed(() => draftArtifacts.value.length)
  const finalArtifactCount = computed(() => finalArtifacts.value.length)
  ...
  const onsiteStatus = computed(() => {
    ...
    return new Date(expires).getTime() > Date.now()
      ? {
          label: `有效至 ${new Date(expires).toLocaleTimeString('zh-CN', { hour12: false })}`,
  ...
  const agentStageCards = computed(() => [
    ...
    },
    {
      ...
      type:
        taskSteps.value.length > 0 && completedStepCount.value === taskSteps.value.length
          ? ('success' as TagType)
          : ('info' as TagType),
    },
    {
      ...
      type:
        pendingAgentApprovals.value.length > 0 ? ('warning' as TagType) : ('success' as TagType),
    },
    {
      label: '工作区',
  ...
  const approvalCount = computed(
    () =>
      store.currentMessages
        ...
        .filter((chunk) => chunk.type === 'ApprovalRequest').length,
  )
  const widgetCount = computed(
    () =>
      store.currentMessages
        .flatMap((message) => message.chunks)
        .filter((chunk) => chunk.type === 'Widget').length,
  )

  const canCreatePlan = computed(() =>
    Boolean(
      store.resolvedSessionId && !store.isSessionTransitionBlocked && !store.isWaitingForApproval,
    ),
  )
  const canRunTask = computed(() =>
    Boolean(
      store.resolvedSessionId &&
      latestTask.value?.canRun &&
      !latestTask.value?.isRunQueued &&
      !latestTask.value?.isRunInProgress &&
  ...
  const canContinueTask = computed(() =>
    Boolean(
      store.resolvedSessionId &&
      latestTask.value?.canRetry &&
      !store.isAgentApprovalAuthorityUnknown &&
      !store.isSessionTransitionBlocked,
    ),
  )
  const hasCurrentTaskWorkspaceProjection = computed(() =>
    Boolean(
      latestTask.value?.workspaceCode &&
      store.currentWorkspace &&
      store.currentWorkspace.taskId === latestTask.value.id &&
