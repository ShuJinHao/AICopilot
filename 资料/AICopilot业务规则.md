...
- `AI-RULE-GOV-001`：所有当前有效约束只能以 `AGENTS.md`、本文档、专题契约、治理清单或部署指南等正式文档为权威来源；项目 `docs/改动复盘与规则沉淀.md` 和工作区 `docs/历史核心记录.md` 只保留历史决策、验证与故障经过，不得成为唯一规则来源。
- 新增长期规则必须使用稳定且唯一的 Rule ID，并在复盘记录权威文件、稳定标题锚点和自动门禁；没有自动门禁时明确写“无”。不得用行号作为锚点，不得改义、复用或静默删除既有 Rule ID。
- 历史复盘不是默认必读材料；只有命中 `AGENTS.md` 的 `AI-HISTORY-001` 触发条件时，才按模块名、Rule ID、错误码或关键类型精准检索相关记录。
- 当前 AI 安全治理和修复执行入口是 `docs/AI架构治理清单.md`；它不是阶段流水，必须逐项记录编号、严重级、状态、验证命令和外部依赖。
- 当前长期专题契约包括 `docs/AICopilot安全部署契约.md`、`docs/Cloud只读数据分析契约.md`、`docs/Agent工作流与异常契约.md` 和 `docs/DDD聚合根边界.md`；触碰部署、Cloud 只读、Text-to-SQL、Agent workflow、MCP/Tool、异常、前端错误、聚合根或上传持久化/安全探测边界时必须先读对应契约。
