...
- AICopilot 后端多服务在同一候选内顺序 publish 时，必须按 service 隔离 .NET SDK artifacts 根，并放在统一输出目录而不是 macOS 临时 detached source worktree；不得共享 `bin/obj`、混用 `/private/var` 与 `/var` 路径别名或靠调整服务顺序规避依赖图污染。
